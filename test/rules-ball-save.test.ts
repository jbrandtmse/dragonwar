...
describe('AC 2 -- the timer starts at the plunge, and the lamp step ladder follows it', () => {
	...
	const ballSaveTicks = shotWindowTicks('ballSaveMs', OVERRIDE_TUNING);
	const hurryUpTicks = shotWindowTicks('ballSaveHurryUpMs', OVERRIDE_TUNING);
	...
	it('ball_launched at tick L emits ball_save_timer_started with untilTick === L + ballSaveTicks', () => {
		...
		expect(started).toMatchObject({ type: 'ball_save_timer_started', untilTick: 3 + ballSaveTicks, tick: 3 });
		expect(result.statesByTick.get(3)!.machine.ballSave.untilTick).toBe(3 + ballSaveTicks);
	});
	...
	it('sampling lampsOf() across the whole window: a non-empty step-1 span, a non-empty step-3 span, every step-3 tick later than every step-1 tick, and off from the displayed expiry onward', () => {
		...
		for (let tick = 1; tick <= untilTick + 5; tick++) {
			...
			const projected = lampsOf({ ...state, tick }, hurryUpTicks).l_ball_save;
			if (tick > untilTick) {
...
describe('AC 3 / AC 4 -- a live drain re-serves instead of ending the ball; the autolaunch is deferred to the served ball\'s arrival', () => {
	...
	it('drain inside grace (past the displayed expiry but before the grace deadline) is saved exactly the same way', () => {
		...
		const result = runRulesScript(close('s_trough_1').at(10 + graceTicks).build(), {
			durationTicks: 10 + graceTicks,
			initialState,
