...
**Escalation rule (implement/qa → opus, per project):** keep the `implement` stage (`bmad-build-auto` on a planned spec) and `qa` (`bmad-qa-generate-e2e-tests`) at `sonnet` for the project's first epic. At each end of epic, `/epic-cycle`'s model-tier checkpoint evaluates the cycle log; if ANY of (a) mean `cr_complete` `unresolved_high_med` ≥ 2 per story with implement at a Sonnet-tier model (the high+med root-cause entries the reviewer could not resolve in its own pass — not the gross count, which an adversarial four-layer review scores at 7–10 on healthy one-pass stories; `cr_complete` entries from before Kit-Version 2026-09-11.1 fall back to `high+med`, logged `metric=gross`), (b) ≥ 2 stories entered the rework loop for language-semantics defects, or (c) ≥ 2 stories' `dev_complete` show `review_loop_iteration ≥ 3` (build-auto's own review-repair loop thrashing), the lead recommends re-pinning those two stages to `opus` for this project. Rationale: two rework iterations (extra Sonnet implement + extra Opus review) already cost more than Opus-implement. Since Kit-Version 2026-08-29.1 the checkpoint **evaluates on every project** (no policy file needed) and always logs `model_tier_checkpoint`; the policy file below plus the user's approval are required only to *apply* a change — the pre-flight `telemetry_gate` re-surfaces any unapplied recommendation at the next run.
