...
3. Assign the retro-review gate: the lowest-numbered epic of the current dispatch batch gets `retro_review: own`; the others get `retro_review: skip (handled by epic {M})`. On the FIRST dispatch of a run, seed the spine id counter: `spine_resolved` in the main checkout (Rule 6), then write `ads+1` to `<worktree_base>/.coordination/spine-next-id` (create-only — never overwrite an existing counter; a resumed run keeps it) and log `spine_counter_seeded next=<N>`.
...
5. Footprint (Rule 11 applies; quote it): `paths_hint` + submodules for this epic; the **contended** list (the union of footprints of every epic that could run concurrently with this one — computed by the orchestrator from the DAG, never by the runner); and the authority statement: "you are the lead for Epic {N}: its story blocks in `epics.md` and everything under `implementation-artifacts` are yours; a path outside your footprint that is not contended may be edited and must be reported under `footprint_extensions:`; contended paths and other epics' stories are a Clarification."
6. Retro-review assignment (`own` or `skip: handled by epic {M}`), and the spine: its path (from the main checkout's `spine_resolved`, re-rooted to the worktree) plus the id counter — "Rule 20 writes go into YOUR worktree's spine at the moment the decision is made, never held for the contract. Claim the next `AD-n` from `<worktree_base>/.coordination/spine-next-id`: create `<worktree_base>/.coordination/locks/spine.lock` atomically (Rule 12 mechanics: content `epic=<N> stage=spine acquired_at=<UTC>`; wait with jitter if it exists), read the number, write number+1 back, delete the lock, use the number. Ids are therefore unique across concurrent epics; the orchestrator reconciles the spines at the merge gate." (The orchestrator seeds `spine-next-id` with `ads+1` from `spine_resolved` before the first dispatch of a run and never edits it afterwards.)
7. Runtime-lock table from parallel.yaml (Rule 12 applies; quote it) + Rule 10 (quote it).
8. Clarification protocol: "If you must stop for ANY clarification, end your run with the `## Clarification Needed` section; you will be resumed with the answer in a follow-up message — your context is preserved. For a missing-tool problem, start the section body with `TOOLING:` and name the exact verification you could not run. Every sha you quote — in that section or in the completion contract — is read with `git rev-parse --short <ref>` at the moment you write it, never recalled from earlier in your context; the orchestrator verifies each one resolves before acting."
9. Completion contract: "End your final message with `## Epic Runner Complete` followed by: `epic: {N}`, `stories_completed: <n>`, `ready_for_merge: true|false`, `submodules_touched: <paths or (none)>`, `head: <sha>` (`git rev-parse --short HEAD` at the moment of writing), `ci: <success|none|unavailable> run=<id>` for that head (resolved, never `pending` — a runner does not complete on a red or unresolved branch; see the base kit's "CI Is a Gate"), `unpushed_work: none` (verify with `git log --branches --not --remotes` in every repo before claiming), `ledger: open_before=<n> resolved=<n> terminal=<n> chartered=<0|1> chartered_size=<n> overflow=<n> reowned=<n> to_decision_sheet=<n> open_after=<n> drain_ratio=<x.xx>` (copied from your `ledger_burndown_*` entry), `decisions_for_user:` (one line per `escalated` / `decision-pending` entry the burn-down gate set aside — `DW-n | <summary> | recommended: <disposition>` — or `(none)`; you never decide these yourself), `amendments:` (one `file:line — what` per Rule 5 apply-and-report amendment you made to a planning artifact or root file, or `(none)`), `footprint_extensions:` (every path you edited outside your footprint, or `(none)`), and a one-line-per-story summary table. Interim or progress messages are NOT a supported return: end your turn ONLY with this contract or with `## Clarification Needed`. Every stage `Agent` call you make is synchronous, so you cannot have 'a stage in flight' at the end of a turn — if you do, you backgrounded or `SendMessage`-resumed a stage, which is forbidden; wait for it, reconcile, and then end properly." Per-story cycle-log entries must carry real `spawn_at` values captured when each stage `Agent` call is made — never backfilled equal to the completion timestamp."

### Notification handling

On each runner notification, classify its final message:

- **`## Epic Runner Complete`, ready_for_merge=true** → **write-ahead first**: append `runner_complete` and `merge_enqueued` to the parallel log BEFORE any merge-gate interaction (a missing completion entry breaks resume and cost attribution — pilot epic-2 gap). Then **verify the contract's facts** with `git -C <worktree>` / absolute paths: `head:` resolves (`git cat-file -e`) and equals the worktree's HEAD; every `amendments:` line exists at the quoted location with its `[AMENDED …]` marker and change-log entry and is Rule 5 tier-1 (an amendment that changed the product promise is a `report_error`-class finding: log it, and put the amendment itself on the decision sheet); every `footprint_extensions:` path overlaps no other open epic's footprint; the `ci:` claim against `gh run list --branch {TICKET}-epic{N} --commit <head> --json databaseId,status,conclusion -L 1` read by the orchestrator itself (`ci_green_verified run= head=`; a red or unresolved head → `merge_gate_blocked reason=ci_red`, and the runner is resumed with the failing job as its next task — the merge gate is not presented). Log `amendments_verified amendments=N extensions=N ok=true|false` and, on any mismatch, `runner_report_error field=<name> reported=<value> actual=<value> impact=<short>` — a phantom sha with a clean, pushed tree is `impact=none`, but it is still logged. Then, if the user is present, proceed to the merge gate (whose decision sheet is built from `decisions_for_user:`); either way, continue dispatching unblocked epics.
- **`## Clarification Needed`** (no `TOOLING:` prefix) → verify every sha it quotes resolves in the worktree (`git -C <worktree> cat-file -e <sha>`; mismatch → `runner_report_error`, and tell the runner the actual value in the answer); log `runner_clarification`; surface the question to the user with Epic + Story context; on answer, `SendMessage` to `epic-runner-{N}` with the answer; log `runner_resumed`. Other epics never pause.
...
1. Precondition: `ci_green_verified` exists for this epic's branch head (notification handling) — never present the merge question on a red or unresolved head. Before the question, surface the runner's `ledger:` line, the overflow it re-owned to next-epic keys (Rule 17), and its `amendments:` / `footprint_extensions:` (verified at notification time); then run the **epic-close decision sheet** (base kit, "Ledger Drain" — Rule 17 (5)) from the contract's `decisions_for_user:` lines, each with the runner's recommended disposition: the user decides every one in this sitting (human-owned items go to `action_items`; a product call may only be deferred to the next X.0 as `owner=<X.0 key>`). Hold the answers — they are written in step 5b. Then ask: "Epic {N} is ready. Merge `{TICKET}-epic{N}` into `<feature>`?" Queue holds while other epics keep running. On the range's FINAL epic, follow the merge with the ledger sweep (`LEDGER slice all` in the main checkout after the merge — every remaining non-terminal entry with a recommended disposition).
...
4. Conflicts: gitlink conflict (`UU <sub>`) → deterministic, resolve via step 3's `git add <sub-path>` (pointer = submodule feature HEAD, which contains all merged epics). `sprint-status.yaml` conflict → take either side, then regenerate with `SPRINT_PLAN generate` (deterministic preserve-never-downgrade merge from the epic files) and re-apply any statuses the losing side carried with `--set`; verify with `SPRINT_PLAN validate` before committing. `deferred-work.md` auto-resolves (union merge attribute). `ARCHITECTURE-SPINE.md` conflict → deterministic, because concurrent runners claimed distinct `AD-n` ids from `spine-next-id`: keep BOTH sides' added AD blocks in ascending id order, both sides' added Conventions/Stack/Deferred rows, and the later `updated:`; an AD whose Rule both sides amended is the only non-deterministic case → STOP and surface. Then `uv run --no-cache .claude/skills/bmad-architecture/scripts/lint_spine.py --workspace <spine-dir>` — any `ad_id` or `ad_fields` finding → STOP (a duplicate id means a runner wrote without claiming; log `protocol_violation`). The spine's `.memlog.md` auto-resolves (union merge attribute — it is append-only). `cycle-log-epic-*.md` never conflicts (one file per epic). ANY code conflict → STOP and surface to the user; auto-resolution forbidden.
5. Verify `git submodule status` (feature heads, no `+`/`-`), push feature. Log `epic_merged_to_feature`.
   5b. **Record the decision sheet** in the MAIN checkout on the feature branch (absolute paths; orchestrator write discipline): one `LEDGER append DW-n "… by=merge_gate …"` trailer line per decision, human-owned items appended to `sprint-status.yaml` `action_items` (owner: the user, `status: open`), post-merge bookkeeping commit + push; log `decision_sheet_recorded decided=N to_action_items=N to_x0=N` in `cycle-log-parallel.md`. In the same commit, **plan the cross-epic routing** (Rule 17 (1b)): `LEDGER slice all` in the main checkout; for every `routed` entry whose owner is a story outside the merged epic and whose `DW-n:` bullet is not yet in `epics.md` (`grep -c "DW-n:" <planning>/epics.md` = 0), insert `- DW-n: <summary> (ledger; routed by <gate> <date>)` under that story's acceptance criteria, at most `routed_story_max` per story; log `ledger_routed_planned story=<key> entries=N excess=N by=merge_gate` and `deps_hash_rerecorded reason=ledger_routed` (an AC insertion is structurally exempt). This needs no contract line — the ledger is the source. The ledger is union-merge, so recording after the merge loses nothing; the answers were taken before it so the user decides everything about Epic {N} in one sitting.
...
- `_bmad-output/implementation-artifacts/cycle-log-parallel.md` (main checkout; same TAB format as per-epic logs; field 2 is `Epic <N>`): stages `lead_model_gate`, `deps_approved`, `deps_hash_rerecorded`, `worktree_provisioned`, `runner_dispatched`, `runner_clarification`, `runner_resumed`, `runner_tooling_backstop`, `runner_interim_status`, `runner_redispatched`, `runner_report_error` (a fact the runner quoted did not match the repository: `field= reported= actual= impact=`), `amendments_verified` (`amendments=N extensions=N ok=`), `spine_counter_seeded` (`next=N`), `spine_reconciled` (SC-4-P step 4: `ads_merged=N lint=ok`), `ci_green_verified` (`run= head=`), `merge_gate_blocked` (`reason=ci_red run= job= head=`), `ledger_routed_planned` (`story= entries= excess= by=merge_gate`), `protocol_violation`, `orchestrator_error`, `runner_complete`, `merge_enqueued`, `epic_merged_to_feature`, `decision_sheet_recorded|skipped` (`decided= to_action_items= to_x0=`), `epic_retro_complete|skipped`, `worktree_removed`, `epic_branches_deleted`, `runtime_lock_swept`, `parallel_summary`. Include `model=` and runner token counts where available; `parallel_summary` story counts include injected X.0 stories.
...
| Clean-tree gate, sprint planning (headless), retro-review/Story X.0 (only if assigned `own`), plan spawns (Opus) + spec validation, implement spawns (Sonnet), QA/CR spawns (synchronous, per the stage→model map), bookkeeping commits (Rule 16), rework loop, AD gates, per-story smoke, per-story commits+pushes (submodules-first, SC-3) | KEPT verbatim — all inside the worktree. Treat the worktree root as `{project-root}` (spawn every stage agent with the worktree root as its working directory — the skill's own bootstrap resolves `{project-root}` from there, so each worktree renders its own `_bmad/render/` generation). Never touch the main checkout or another epic's worktree. |
...
| AD-tooled verifications (the architecture spine, Rule 6; the lock stage keeps its `adr_verifications` spelling) + smoke + QA e2e execution (+ the implement stage if `runtime_locks[].stages` lists `implement` / `all-runtime`) | KEPT, but wrapped in the runtime lock when parallel.yaml lists the stage (Rule 12). On a missing tool: `TOOLING:` clarification (the orchestrator runs it and resumes you). |
| Ledger drain: harvest-with-owner, per-story `ledger_adjudicated`, and the end-of-epic **burn-down gate** (`ledger_burndown_*`, including chartering and running the burn-down story `N.<next free number>` — bounded by `burndown_story_max` — through the full pipeline inside the worktree) | KEPT verbatim — runner-side, because the burn-down story is part of Epic N and must land on `{TICKET}-epic{N}` before the merge. The ledger lives inside the worktree (it merges with `merge=union`; `LEDGER` only ever adds lines). Re-own the overflow to specific next-epic story keys as Rule 17 requires; the orchestrator surfaces those at the merge gate. The epic-start `ledger_load` runs in the worktree too (its `owner:none` re-own happens there). **The decision sheet is NOT yours:** `escalated` / `decision-pending` entries are set aside, listed under `decisions_for_user:` in the contract with a recommended disposition each, and decided by the user at the orchestrator's merge gate — a runner never re-owns, closes, or answers one. |
| Rule 5 planning amendments and footprint | KEPT with Rule 11's authority split: tier-1 (intent-preserving) amendments to your own epic's story blocks and root-file bookkeeping are applied under the amendment discipline and listed under `amendments:`; paths outside your footprint that no contended epic owns are edited and listed under `footprint_extensions:`; tier-2 (product-changing) amendments, contended paths, other epics' stories, new dependencies/licences are `## Clarification Needed`. The orchestrator verifies every listed amendment post-hoc. |
| Spec-validation decision dependency (base kit) | KEPT — a spec that builds on a `decision-pending` entry is a `## Clarification Needed` from you at plan time, not something to carry to the decision sheet. |
| Ledger inbox at the plan spawn + `owned_ledger=` at spec validation; `DW-n:` acceptance bullets for entries routed to THIS epic's stories (Rule 17 (1b)) | KEPT verbatim, in the worktree — your own epic's story blocks are yours (Rule 11). An entry you route to ANOTHER epic's story gets its bullet from the orchestrator post-merge (it reads the ledger; nothing to report). |
| `committed … ci=` and the CI resolution points (base kit "CI Is a Gate") | KEPT, in the worktree: record the run at every push, resolve before the next plan spawn and at the burn-down gate, handle a red per the base kit; your completion contract's `ci:` line must be a resolved `success` (or `none`/`unavailable`) — never complete on `pending` or `failure`. The orchestrator re-reads it before the merge gate. |
| Architecture spine: `spine_resolved` at epic start, the path in every spawn prompt, Rule 20 light-path writes (new/amended `AD-n`, Conventions/Stack rows, `memlog.py append`, `lint_spine.py`) | KEPT, **at the moment of the decision, inside the worktree** — the spine is a tracked planning artifact, so the write lands on `{TICKET}-epic{N}` in the same bookkeeping commit as the decision and survives a runner crash, a mid-run upgrade, or a declined merge; a decision held for the completion contract does not. The one thing a worktree cannot know is the next free `AD-n`, so claim it from the orchestrator's counter (skeleton item 6: `spine.lock` + `spine-next-id`, Rule 12 mechanics) before writing; ids stay unique across concurrent epics and are never renumbered. Amending an existing AD's Rule or a Conventions/Stack row needs no claim. The orchestrator reconciles spines at SC-4-P step 4 (deterministic) — a spine edit is not an `epics_md_hash` event. A decision that conflicts with an existing AD is never resolved by a runner: it is a `## Clarification Needed`; the Update intent runs orchestrator-side, interactively, with runners paused. |
| End-of-epic: retro question, decision sheet, SC-4 merge, `epic-{N}: done` write, IDE-sync windows | REMOVED — orchestrator-owned (the decision sheet is built from your `decisions_for_user:` lines and recorded post-merge on the feature branch; the retro runs post-merge, orchestrator-side, and receives the `ledger:` line from your completion contract as its argument context). Your last acts: burn-down gate logged, final story committed AND pushed, verify `unpushed_work: none` in every repo, append `epic_runner_complete` to your cycle log, emit the completion contract. |
...
- **Treating `paths_hint` as authority** — the footprint exists so two runners do not collide; it says nothing about what a runner may decide. A runner is the lead for its epic (its story blocks, its artifacts, Rule 5 tier-1 amendments, non-contended root files — applied and reported), and a Clarification is for contended paths, other epics' stories, and product-changing amendments (Rule 11). Halting for permission to apply an already-verified, intent-preserving amendment buys latency, not safety; the post-hoc `amendments_verified` check is where the safety lives.
- **A runner deciding its own decision sheet** — `escalated` / `decision-pending` entries reach the user through `decisions_for_user:` in the contract and the orchestrator's merge gate. A runner that re-owns or closes them at its burn-down gate has made the user's call and buried it in a trailer line.
