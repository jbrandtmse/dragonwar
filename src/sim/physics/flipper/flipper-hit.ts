...
// Source: lib/vpt/flipper/flipper-hit.ts
//
// Deviation: dropped the `getInstance(data, state, events, physics, table)`
// static factory entirely -- it resolves the flipper's surface height and
// radius-min/max difficulty scaling through `Table`/`FlipperData` (VPX's
// table-loading and per-difficulty override system, out of scope per AD-1).
// `sim/physics/flippers.ts` builds a `FlipperHit` from the authored
// `FlipperConfig`/`FlipperPhysicsData` (`flipper-config.ts`) directly via the
// constructor below instead.
// Deviation: dropped the `tableData: TableData` and `physics: PlayerPhysics`
// constructor parameters that only existed to construct the (also dropped)
// internal `FlipperMover` with them -- see `flipper-mover.ts`'s own header.
// Deviation: dropped `updatePhysicsFromFlipper()` and its constructor call.
// Upstream resolves elasticity/elasticityFalloff/friction/scatter from
// `FlipperData`'s own (possibly override-physics) fields; this port instead
// leaves those at `HitObject`'s own defaults and expects the caller
// (`sim/physics/flippers.ts`) to apply `TUNING.materials.flipper_rubber`
// through `setElasticity()`/`setFriction()`/`setScatter()` -- the exact same
// calls `sim/physics/loader/index.ts`'s `applyMaterial()` already makes for
// every other hit object in this compound body, rather than a second,
// flipper-only material-resolution path.
...
import { elasticityWithFalloff } from '../functions';
...
export class FlipperHit extends HitObject {
	...
	constructor(config: FlipperConfig, data: FlipperPhysicsData, state: FlipperState, events: EventProxy) {
		super();
		this.events = events;
		this.mover = new FlipperMover(config, data, state, events);
		this.data = data;
		this.state = state;
		// Deviation (addition, not upstream): upstream's own FlipperHit never
		// calls setType() at all (objType stays CollisionType.Null, verified
		// against the pinned source), but CollisionType.Flipper exists
		// specifically to identify this hit shape's kind -- set here so a
		// future consumer (e.g. contact-sound routing by objType) does not
		// have to re-derive "this HitObject is a flipper" some other way.
		this.setType(CollisionType.Flipper);
	}
