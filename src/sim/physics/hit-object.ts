...
// Source: lib/physics/hit-object.ts
//
// Deviation: dropped `applyPhysics(data: IPhysicalData, table: Table)`. It resolved a
// named material through `Table.getMaterial()` (the vpx-js table/item-loading system,
// out of scope per AD-1) and then called the very setters kept below. DragonWar's own
// table tunables (`sim/table/tuning.ts`, Story 1.3+) will call `setElasticity()` /
// `setFriction()` / `setScatter()` directly instead; this story's harness does the
// same. See docs/spikes/spike-1.md for the full deviation list.
