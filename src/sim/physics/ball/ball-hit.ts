...
// Source: lib/vpt/ball/ball-hit.ts
//
// Deviations (see docs/spikes/spike-1.md for the full list):
//  - `TableData` (`lib/vpt/table/table-data.ts`, out of scope per AD-1) is replaced
//    with the minimal structural `BallHitTableData` type below, carrying only the two
//    fields this file reads (`tableHeight`, `globalDifficulty`).
//  - The hardcoded ball-ball restitution `0.8` at line 303 of the upstream file is
//    replaced by the named, verbatim `BALL_BALL_RESTITUTION` constant in
//    `constants.ts` (AD-15; this story's task list calls for this extraction).
//  - `collide3DWall()`'s scatter draw (`Math.random() * 2 - 1`) is replaced with a
//    deterministic stand-in. DragonWar's physics draws no randomness — scatter is 0
//    on every material by default (AD-3) — so `scatterAngle > 1.0e-5` is never true
//    and this branch is dead in every scene this story builds; the literal
//    `Math.random` token is still banned under `sim/` by AD-16 regardless of
//    reachability. A seeded PRNG in physics state replaces this if scatter is ever
//    enabled (Story 1.3+ territory, per AD-3).
...
import { elasticityWithFalloff, HARD_SCATTER } from '../functions';
...
export interface BallHitTableData {
	tableHeight: number;
	globalDifficulty: number;
}

// Deterministic stand-in for upstream's `Math.random()` scatter draw — see the
// deviation note above this file's header.
function deterministicScatterUnit(): number {
	return 0;
}
...
export class BallHit extends HitObject {
	...
	private readonly tableData: BallHitTableData;
	 ...
	 */
	constructor(ball: Ball, data: BallData, state: BallState, initialVelocity: Vertex3D, tableData: BallHitTableData) {
		super();
...
//#endif
		...
		const impulse = -(1.0 + BALL_BALL_RESTITUTION) * dot / (myInvMass + ball.hit.invMass);
...
//#endif
		...
		if (dot > 1.0 && scatterAngle > 1.0e-5) {          // no scatter at low velocity
			let scatter = deterministicScatterUnit() * 2 - 1;           // -1.0f..1.0f
