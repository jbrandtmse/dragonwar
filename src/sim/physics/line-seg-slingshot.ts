...
// Source: lib/physics/line-seg-slingshot.ts
//
// Deviation: upstream takes a `Surface`/`SurfaceData` pair from `lib/vpt/surface/`
// (the vpx-js table-item system, out of scope per AD-1). The `surface` parameter was
// never read anywhere in this file's body, so it is dropped; `surfaceData` is
// replaced with the minimal structural `SlingshotSurfaceData` type below, carrying
// only the two fields this file actually reads. Not exercised by Story 1.1's harness
// (no slingshot in the spike scene) — ported to complete lib/physics/'s closure per
// the Code Map, for later stories to build hardware rules on top of. See
// docs/spikes/spike-1.md for the full deviation list.
...
export interface SlingshotSurfaceData {
	isDisabled: boolean;
	slingshotThreshold: number;
}

export class LineSegSlingshot extends LineSeg {

	private readonly physics: PlayerPhysics;
	private readonly surfaceData: SlingshotSurfaceData;
	private slingshotAnim = new SlingshotAnimObject();
	public force: number = 0;
	private eventTimeReset: number = 0;
	public doHitEvent: boolean = false;

	constructor(surfaceData: SlingshotSurfaceData, p1: Vertex2D, p2: Vertex2D, zLow: number, zHigh: number, physics: PlayerPhysics) {
