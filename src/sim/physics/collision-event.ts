...
// Source: lib/physics/collision-event.ts
...
export class CollisionEvent {
	...
	public static reset(event: CollisionEvent): void {
		// TS 7's `delete` requires the property to be declared optional; `ball` is
		// typed non-optional everywhere it's *read* (every real consumer sets it via
		// `claim()`/the constructor before use), so the cast is local to this one
		// pool-hygiene line rather than widening the field's type project-wide.
		delete (event as { ball?: Ball }).ball;
		delete event.obj;
