...
**Requires BMAD Method v6.11.0 or later and base kit ≥ 2026-08-30.2** (the v6.11 `bmad-build-auto` pipeline). Runners spawn `bmad-build-auto` stage agents, which spawn their own subagents — depth-3 nesting (orchestrator → runner → build-auto → handoff/review) verified on Claude Code 2026-08-26.
...
| `_bmad/custom/skill-rules.md` | append | Rules 10–12 (submodule discipline, footprint discipline, runtime lock) — Rules 13–20 come from the base kit |
...
| `.gitattributes` | append | union merge for `deferred-work.md` and the spine's append-only `.memlog.md` |
...
- `.gitattributes`: `_bmad-output/planning-artifacts/architecture/**/.memlog.md merge=union` (the spine's append-only decision log; `ARCHITECTURE-SPINE.md` itself is NOT union-merged — its conflicts are reconciled deterministically at SC-4-P step 4)
...
grep -cE "^## Rule (1[3-9]|20)" _bmad/custom/skill-rules.md                     # 8 (working-dir, ASCII-escape, disposition bar, clean-tree, drain, containment, falsifiability, spine — base kit)
grep -c "spine-next-id" .claude/commands/epic-cycle.md                          # >= 3 (dispatch loop seed, skeleton item 6, Runner-Mode Deltas)
grep -c "lint_spine" .claude/commands/epic-cycle.md                             # >= 2 (base kit + SC-4-P spine reconciliation)
grep -c "ci_green_verified" .claude/commands/epic-cycle.md                      # >= 3 (base SC-4 + notification handling + merge queue)
grep -c "ledger_routed_planned" .claude/commands/epic-cycle.md                  # >= 3 (base kit + merge queue 5b + state-file stage list)
grep -c "deregistered and empty" _bmad/scripts/remove-epic-worktree.sh          # 1 (Windows handle-lock tolerance)
grep -c "concurrency is not authority" _bmad/custom/skill-rules.md              # 1 (Rule 11, this kit-version)
grep -c "contended" .claude/commands/epic-cycle.md                              # >= 2 (skeleton item 5, Runner-Mode Deltas)
grep -c "decisions_for_user" .claude/commands/epic-cycle.md                     # >= 4 (contract, notification handling, merge queue, Runner-Mode Deltas)
grep -c "amendments_verified" .claude/commands/epic-cycle.md                    # >= 2 (notification handling, state-file stage list)
grep -c "runner_report_error" .claude/commands/epic-cycle.md                    # >= 2
grep -c "decision_sheet_recorded" .claude/commands/epic-cycle.md                # >= 5 (base kit + merge queue 5b + state-file stage list)
grep -c "footprint_extensions" .claude/commands/epic-cycle.md                   # >= 3
...
grep -c "merge=union" .gitattributes                                            # >= 2 (deferred-work.md + the spine's .memlog.md)
grep -c "\.worktrees/" .gitignore                                               # >= 1
grep -c "escalated" .claude/commands/epic-cycle.md                              # >= 3 (Rule 15 dispositions)
grep -c "Rule 19" .claude/commands/epic-cycle.md                                # >= 4 (base kit ≥ 2026-08-30.1 present)
