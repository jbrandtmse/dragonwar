...
**Requires BMAD Method v6.11.0 or later and base kit ≥ 2026-08-30.1** (the v6.11 `bmad-build-auto` pipeline). Runners spawn `bmad-build-auto` stage agents, which spawn their own subagents — depth-3 nesting (orchestrator → runner → build-auto → handoff/review) verified on Claude Code 2026-08-26.
...
| `_bmad/custom/skill-rules.md` | append | Rules 10–12 (submodule discipline, footprint discipline, runtime lock) — Rules 13–19 come from the base kit |
...
grep -cE "^## Rule 1[3-9]" _bmad/custom/skill-rules.md                          # 7 (working-dir, ASCII-escape, disposition bar, clean-tree, drain, containment, falsifiability — base kit)
