...
| `_bmad/custom/skill-rules.md` present | Back up to `<file>.bak-<UTC>`. Before overwriting in Step 2, extract and PRESERVE: (a) the parallel kit's `## Rule 10` – `## Rule 12` blocks, re-inserted immediately before `## Rule 13` in the new file (so a parallel install is not silently un-installed); (b) everything under `## Project-specific rules` (rules numbered 20+ and any prose), re-appended verbatim. Report what was carried over. |
...
(Field report 2026-08-29: five classes of vacuous test across one epic — every one found by adversarial review or by a later story, none by a gate designed to catch it; the lead's own ADR gate recorded `pass` for an assertion typed `readonly never[]`. Test count grew 518 → 659; discriminating power did not track it.)

## Project-specific rules (add below as retros surface them)

> Add additional rules here as retrospectives identify durable patterns. Number sequentially after Rule 19. Each rule should state what it applies to, the obligation, and (briefly) why.
````
...
Same pattern as File 2: the four review layers are the skill's v6.11.0 defaults plus Rule 18 containment (these layers only read, so the containment is short), the softened `blind-hunter` quota, and Rule 19's falsifiability brief on `verification-gap`. Re-check against `.claude/skills/bmad-code-review/customize.toml` after a BMAD upgrade.
...
3. **Lead** gates the spec: Integration-AC validation (Rules 1/2), ADR mapping (Rule 6), decision dependency (a spec that builds on a `decision-pending` ledger entry is an intent gap for the user NOW, not at the merge gate); logs `spec_validated`; sets the story `ready-for-dev` via `SPRINT_PLAN generate --set`; bookkeeping commit (everything under `implementation-artifacts`: spec, log, tracker).
...
5. **Lead** executes any ADR-tooled AC verifications, each with a named, applied, and reverted mutation (Rule 19 — a pass without one is `unverified`; see "ADR-Aware Execution").
...
| --- | --- |
| Fix it — in a named story | `status=routed owner=<story key> by=merge_gate note=<decision>` |
...
4. Re-spawn the same stage's agent with the clarification baked in (prompt, or the spec's `## Design Notes` — committed first, Rule 16). Never `SendMessage` the old agent (see Anti-Patterns).
...
- `spec_validated`: `service_introducing=true|false integration_ac=<present|declared-none|user-waived> adr_constrained_acs=<csv-or-empty> decision_dependency=<DW-n csv|none>`.
...
1b. **Finding disposition bar (Rule 15), Rule 16, and the v6.11 stage doctrine installed:**
   ...
   grep -cE "^## Rule 1[3-9]" _bmad/custom/skill-rules.md   # 7 (13, 14, 15, 16, 17, 18, 19)
   grep -c "Rule 19" .claude/commands/epic-cycle.md                      # >= 4 (Task Sequence, QA block, CR block, ADR gate, anti-pattern)
