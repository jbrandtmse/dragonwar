...
### AD-6 — Physics owns ball bodies and mechanical state; rules own ball accounting; devices park and eject only on command `[ADOPTED]` `[AMENDED 2026-09-11 — stray balls are cleared before every serve, so the ball controller becomes a second RecoverCommand issuer (author decision, DW-244); the overflow eject now cross-references AD-18's Lock phasing]`
