...
### AD-16 — Boundaries are linted in CI by a TypeScript-API-free tool; ported and borrowed files keep their notices `[ADOPTED]` `[AMENDED 2026-08-30 — named check:headers as the single licence-header authority]`
...
| Licence headers | Ported: original header retained + `// Ported from vpdb/vpx-js (GPL-2.0-or-later); distributed with DragonWar under GPL-3.0`; new: GPL-3.0 header. Enforced by `pnpm check:headers` alone (AD-16); no test duplicates the check. |
