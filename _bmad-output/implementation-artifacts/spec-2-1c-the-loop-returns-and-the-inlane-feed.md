...
- **AC 3** -- Given the shot map drawn in Story 2.1b, when a ball is driven into the Left Loop or the Right Loop at a plausible flipper-shot speed from a release point verified clear of every footprint and outside every zone under test, then the completed Loop delivers the ball to the **inlane on the corresponding side** -- `s_inlane_l` or `s_inlane_r` closes -- and it then arrives **playable at that side's bat band**, rather than descending an outlane. **[AMENDED 2026-09-03 -- the Ramp clause is MOVED OUT of this AC, into Story 2.1f's block.]** This AC used to end "and the Ramp's return delivers to the right inlane, closing `s_inlane_r` (`OQ-6`/FR-27, decided and now delivered)". That clause could be satisfied only through `driveShot()`'s teleport: the Ramp channel is unreachable by any shot from below (`DW-137` -- entering it needs a ball centre >= 351.495 while `col_sling_r`'s band admits only <= 300.505; 256 swept releases close `s_ramp_enter` zero times), and it has NEVER been reachable in committed geometry -- 21.990 mm short at this story's own baseline `43a9c37`, deepened to 50.990 mm here as the granted consequence of widening the Loop lanes for the orbit. The Ramp's return *geometry* (the crossing over the Right Loop) is built and verified; what is not delivered is a player being able to reach it, which needs the bottom-right corridor re-solved as a whole. Moved by the author's decision so this story does not close on an acceptance criterion it does not meet.
...
#### 2026-09-03 -- code review pass 3, the RE-REVIEW of the rework (`review_mode: full`; layers: Blind Hunter, Edge Case Hunter, Verification Gap Reviewer, Acceptance Auditor -- all four run, no model override)

Diff reviewed: `git diff 43a9c37` (the ORIGINAL iteration-1 baseline, so the whole story is in scope, not just the fix). 24 files, +3211/-681, tree clean, everything committed. No layer edited, committed or pushed anything (`git status --short` clean and `git log --branches --not --remotes` empty throughout).

**Verdict on the rework: eight of the nine pass-2 findings are genuinely closed, and both mechanisms this re-review was asked to scrutinise hold up.** But an independent sweep found one HIGH the previous passes did not: the right-side approach corridor.

**Independent evidence gathered by this pass (measured here, not re-read from the record):**

- Full suite with `BLENDER` exported: **87 files / 1219 passed / 0 failed**, matching the record exactly. `pnpm typecheck` / `lint:boundaries` / `check:headers` / `check:attributions` all clean. `pnpm check:ad7` still exits 1 naming `AD-7` / `DW-70` / `bd_trough`. `TABLE.shots` still exactly `{}` (`src/sim/table/dragonwar.ts:335`). Rule 14: every changed source file is byte-ASCII.
- Rule 1: the "no new service" claim is **true** -- `git diff --stat -- src/` is empty. AC 3 is observed at `createMachine().step()`, AC 4/AC 8 through `runReplay()` and the golden headers. Rule 3 is satisfied by those same real-runtime harnesses (the geometry never reaches the glb, so the simulation IS its target runtime).
- Rule 6: AD-2, AD-3, **AD-6 as amended**, AD-7, AD-10, AD-11, AD-15, AD-16, AD-17 and AD-19 each cross-checked against the delivered diff. The AD-6 pass-through spinner amendment and the OQ-6 orbit refinement are both present in the spine. **No AC contradicts an AD's Rule.**
- A full pairwise convex-clip sweep of all 69 `col_` footprints with z-overlap: **no body-on-body interpenetration**. The only 14 overlaps are guide-to-its-own-terminating-post, by design. The rework introduced none.
- **The `col_loop_top` ridge is genuinely and tightly pinned** -- the thing this re-review was told to look hardest at. Verified by mutating the committed collision document three ways and reverting each (SHA-256 `4955324f...` byte-identical after every cycle): flattening the ridge to a quad turns **exactly** the two `col_loop_top` descending-release columns red and nothing else in the file, while `test/asset-contract.test.ts` stays 44/44 green; steepening it to `RIDGE_DROP_MM` = 5.0 turns the **Left Loop's own 34 mm entry offset** red, reproducing the regression the generator's sweep records; and setting it to zero or reversing it into a valley is rejected outright by the loader's strict-convexity gate (DW-52). The shipped 2.5 mm is bracketed on both sides by real, discriminating tests. The 5-point footprint is convex (checked by hand, all five cross products positive) and the shipped document matches the generator exactly.
- The ridge's **peak is an exact unstable equilibrium**, as its own comment says: a ball set at rest at x = 234.2 makes 0.05 mm of progress in 500 ticks, while 1 mm either side rolls off normally (26 mm). Measure-zero and honestly documented; not filed.
- **`roll-and-drain`'s retime is correct and honestly recorded.** `durationTicks` 9282 sits between its own drain (9281) and the no-press control's (9284). The margin is genuinely one tick on the low side and the notes say so plainly. Both failure directions fail **loudly** (a late drain breaks the scenario assertion; an early control drain collides the parity hashes), so this is a maintenance cost, not a latent vacuity. No threshold lowered, no `PARITY_INERT` entry added.
- **`col_loop_r_deflector`'s 34 mm dead-flat north cap genuinely parks a ball** (measured 0.01-0.49 mm over 500 ticks at three columns) and has no descending-release column -- but it is **unreachable**: across the real autolaunch plunge and lane drives at 1800 / 2100 / 2400 / 2497 / 2548 mm/s (the table's own measured maximum), a ball in the shooter lane peaks at y ~= 983, some 47 mm below the cap's own rest height. Closed `wontfix-theoretical` below.

**Blocking finding (HIGH) -- this returns the story to `in-progress`:**

- [ ] [Review][Decision] **HIGH (fix-risk: high) -- the re-sited `col_sling_r` walls off the whole right-side approach corridor, and the consequence was ledgered on premises the diff falsifies.** `col_sling_r` moved from x[360.000, 410.000] to x[314.000, 370.400] **in this story** (measured against baseline `43a9c37`'s own committed document), i.e. its west face -- the corridor's east boundary -- moved 46 mm west. Two measured consequences: **(a) the DRAGON bank.** The corridor from `col_guide_outer_r`'s east face (279.525, genuinely unmoved) was **80.475 mm** before this story and is **34.475 mm** after. Direct-from-below reachability fell from **four** of six targets (baseline bank D 255-266 ... N 325-336 against free ball centres 293.020-346.505) to **two** (G, O). **(b) the Ramp, which is worse.** To enter the Ramp channel at y = 485 a ball centre must lie in [351.495, 358.505]; passing the slingshot's own band (y 420..455) admits centres only up to 300.505 (it was 346.505). The shortfall was 21.990 mm before this story and is **50.990 mm** after -- so the Ramp was already unreachable and this story made it ~29 mm more so. Demonstrated against the real physics pipeline: **256 swept releases from below the sling band (x 285..360 step 5, aim -20..+30 deg, 1800 and 2400 mm/s) close `s_ramp_enter` ZERO times.** `test/shot-routing.test.ts`'s Ramp case -- AC 3's own OQ-6/FR-27 clause -- passes only because `driveShot()` repositions the ball at (355, 465), inside a ~2 mm slot above the slingshot that no shot can reach. This is the same "physically unreachable behind a shadow" failure the DRAGON-bank rework in this very pass exists to prevent, applied to the body everything else was moved to accommodate. Compounding it, `SLING_R_X0_MM = 314.0` shipped as a **bare literal with no derivation** (AC 5 / the Always rule), and the deferral rests on "neither boundary moved" / "the corridor itself did not move" / "`col_sling_r`'s own position is Story 2.2's hardware, explicitly out of this story's own grant" -- all three false, the last contradicted by this spec's own **Explicit permission** block, which names `col_sling_r` among the bodies this story MAY move. Under Rule 15 a consequence this story introduced is in-footprint and is not eligible for the "ledgered, not blocking `done`" disposition. **The comments and the ledger are corrected by this review; the geometry is not** -- moving the sling back east re-narrows the Right Loop lane the orbit needs, so the right-side budget (sling span, Ramp walls, DRAGON bank, loop lane) has to be re-solved together, which is a lead/product call of exactly the shape this spec's own Block If contemplates. Ledgered **DW-137** (Ramp) with a correcting trailer on **DW-136** (bank). [`tools/make-placeholder-blend.py` `SLING_R_X0_MM`; `test/shot-routing.test.ts` Ramp case]

**Applied by this review (patch findings)** -- comment, gate and test-assertion changes only. No geometry, no re-export, no golden trajectory and no hash moved; full suite re-verified **87 files / 1219 passed / 0 failed** afterwards.

- [x] [Review][Patch] **MED -- `SLING_R_X0_MM = 314.0` carried no derivation**, the only bare literal in its block and the most consequential figure this story moved on the right side. Added, with the baseline-vs-HEAD measurement and both consequences above. [`tools/make-placeholder-blend.py`]
- [x] [Review][Patch] **MED -- the DRAGON-corridor comment asserted the corridor "did not move" and that `col_sling_r` was not this story's to move.** Both false (measured above); the second is contradicted by the spec's own Explicit permission block. Corrected in place. [`tools/make-placeholder-blend.py`]
- [x] [Review][Patch] **MED -- `assertLoopMissOutcome()` asserted nothing at all when `s_shooter_lane` closed.** A bare `return` skipped **both** `assertNotStranded` and `assertNotStillInPlay`, so the fourth row of the new off-column sweep -- the only row that takes that branch -- was left with no assertion beyond `assertReleaseClear()`, and any future run that closes that switch and then strands elsewhere would pass. The sneak-back is a legitimate *specific* outcome, so it is now pinned as one: the ball must come to rest inside the shooter lane. Found by three layers independently. [`test/shot-routing.test.ts` `assertLoopMissOutcome`]
- [x] [Review][Patch] **MED -- the pass-2 DRAGON leg-shadow HIGH fix shipped with no regression gate.** Every existing gate is blind to it by construction (the Lock-lane width reads the left leg's east edge to the right leg's west edge and the right leg grows eastward; the centreline was deliberately re-derived from `sw_lock_lane`; the outer-to-outer span is independent of X0; the bank's routing case asserts only "at least one target closes"), so reverting `DRAGON_LEG_R_W_MM` 45 -> 60 reburies `col_dragon_d` with the whole suite green. Its sibling finding from the same pass (`col_top_divider_4`) DID get a gate; this is the matching half. Added, with its `// mutation:` comment, and **demonstrated red** (mutated the committed document to widen the leg: "expected -10 to be greater than 0", that test alone failing; reverted, SHA-256 byte-identical). [`test/asset-contract.test.ts`]
- [x] [Review][Patch] **MED -- the `col_loop_turn_l/_r` adjacency gate names a falsifier that cannot falsify it.** Its comment offers "`PLAYFIELD_H_MM`'s own definition" as a mutation, but `col_wall_top` is authored AT `PLAYFIELD_H_MM` and both turn prisms take `PLAYFIELD_H_MM` as their high corner, so that constant moves both sides of the comparison equally and the assertion stays green for every value of it. Only a single-site source edit can separate them. This is the same class pass 2 corrected twice in this block, still half-wrong in the same direction. Corrected; the `col_ramp_turn` sibling is genuinely fine and is noted as such. [`test/asset-contract.test.ts`]
- [x] [Review][Patch] **MED -- the `s_spinner` assertion's `mutation:` comment names `col_spinner_l`, which cannot control it.** Under the amended AD-6 the node is intentionally non-colliding and `s_spinner` closes purely from the analytic `sw_spinner` zone -- the generator's own note records that DELETING `col_spinner_l` leaves all three Left Loop cases green including this line. The assertion is real coverage, but of the zone against the ascending column, not of the `col_` node's placement; a future editor (2.1d renames it `vis_spinner_l`) would read it as a guard on the node and get a false green. Re-worded to the falsifier that matches the mechanism. **This is not a re-file of the by-design spinner** -- it is the shipped mutation claim. [`test/shot-routing.test.ts`]
- [x] [Review][Patch] **LOW -- `tools/make-placeholder-blend.py` and this spec cite DW-135/DW-136 swapped relative to the ledger.** The ledger (authoritative) has DW-135 = `col_spinner_l` (`by-design`) and DW-136 = the DRAGON corridor (`routed` to 2.3); the generator said the reverse at both sites. A reader following a source comment landed on the wrong entry. Corrected in the generator. [`tools/make-placeholder-blend.py`]
- [x] [Review][Patch] **LOW -- the descending-sweep comment still described `col_loop_top`'s north face as "deliberately left flat ... argued safe ... rather than tested"** in the very comment introducing the two columns the ridge now serves, after the same pass replaced that face with the ridge. Corrected, and the measured falsifiability bracket recorded next to it. [`test/shot-routing.test.ts`]
- [x] [Review][Patch] **LOW -- two comments still quoted `DRAGON_BANK_X0_MM 255 -> 235` against the shipped 240**, the intermediate value the rework superseded. Completed. [`test/shot-routing.test.ts`]

**Deferred / ledgered (not blocking, dispositioned at review time per Rule 15):**

- [x] [Review][Defer] **MED (fix-risk: med, in-story) -- the RIGHT inlane feed is behaviourally inert, and its bat-side end lands on the wrong side of the pivot.** Measured per tick against the real pipeline: the closest approach of all three Left Loop orbit shots to `col_guide_inlane_feed_r`'s **polygon** is 13.515 / 13.566 / 13.589 mm against a 13.495 mm ball radius -- **never contact**; the ball crosses the right bat band ballistically, having taken its leftward velocity higher up. Its left mirror does contact, at 13.488 / 13.488 / 13.489 mm -- inside by 0.006 mm. So the two sides straddle the contact threshold by hundredths of a millimetre, which is coincidence, not clearance. Structurally: `INLANE_FEED_R_X1_MM = 356.0` sits 11.6 mm **outboard** of the right pivot (344.4) on the base circle, where `INLANE_FEED_L_X1_MM = 175.0` sits 5 mm onto the blade; the new "each inlane feed ramp ends over its own bat" gate passes by 0.9 mm because `col_flipper_r`'s bbox includes that base circle. And the behavioural pin cannot see any of it: shifting the rail -20 / +20 / +40 / +60 mm leaves all 38 routing cases green (demonstrated, document reverted each time). **DW-130's behavioural half is therefore still open**, contrary to this file's own pass-2 note that the rail "can no longer be shifted or deleted with every routing case staying green" -- the genuine closure is the new dimensional gate (which does go red at +20), not the band's `yMax` change. Root-cause trailer appended to DW-130 rather than a new entry.
- [x] [Review][Defer] **LOW -- `col_loop_r_deflector`'s 34 mm dead-flat north cap is a genuine DW-119 surface with no descending-release column, but is unreachable.** Closed `wontfix-theoretical`. `reopen_if`: any trace shows a ball with x > 480.4 and y > 1025.
- [x] [Review][Defer] **LOW -- `RIDGE_DROP_MM` and `loop_top_peak_x` are the only load-bearing figures this story adds that live inside `main()` rather than the constants block, with no dimensional gate and no `// mutation:` line**, against task 15's convention for every sibling figure. Closed `wontfix-accepted`: the behavioural bracket measured above is strictly stronger than a dimensional gate would be, the loader forecloses the degenerate and reversed cases, and the failing test names the body. `reopen_if`: a `col_loop_top` regression ever reaches a reviewer as an opaque routing failure rather than a named one.
- [x] [Review][Defer] **LOW -- the two new `test/flipper-sweep-clearance.test.ts` cases carry bare 12 mm floors with no recorded measurement**, though the file's own convention (and its 2.1a pair, 14.1945 / 17.0770 mm) is to record the measurement the floor sits under. Closed `wontfix-accepted`; `reopen_if`: a feed-rail move ever passes these floors while entering the bat's swept envelope.

**Consulted and NOT re-filed:** DW-70 / AD-7 / `bd_trough` (by design, Story 2.5), DW-82 (6.7), DW-119 (the ridge is a new instance, closed), DW-121, DW-123 (delivered), DW-126, DW-127, DW-128, DW-129 (frozen intent-contract wording -- adjudicated), DW-131, DW-132, DW-133, DW-134, **DW-135** (`col_spinner_l`, closed `by-design` under the amended AD-6 -- explicitly not re-opened), DW-136 (corrected, not re-filed).

**Examined and dismissed (no action):**

- `col_spinner_l` as dead geometry -- settled `by-design` by the AD-6 amendment; the spine, `docs/decisions.md` and the ledger all agree, and Story 2.1d owns the `vis_` rename.
- The frozen `<intent-contract>` I/O Matrix rows 1-2 -- DW-129, adjudicated; the block is read-only to this pipeline.
- AC 2 and AC 8 carrying no `mutation:` line -- by design, and `## Verification` says why.
- A layer's claim that `sw_loop_l_in` does not cover the lane mouth -- the zone demonstrably closes on every tested entry offset and on the orbit's own descent; the claim mis-models the entry column.
- Two layers reported transient modifications to `public/assets/dragonwar.collision.json` and a stray `test/zz-cr-diagnostic.test.ts` mid-review. Those were **this reviewer's own sanctioned mutation demonstrations and diagnostics**, not layer violations. Every geometry figure in this section was taken or re-confirmed against the reverted, byte-identical document.
