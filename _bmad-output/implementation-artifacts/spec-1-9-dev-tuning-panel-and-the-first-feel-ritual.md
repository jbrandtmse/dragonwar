...
baseline_commit: '5aa86165b626a22581c5afe7cefba1ceff516ea0'
...
*`/bmad-code-review`, 2026-08-30. Review tier `full-opus` (no
`_bmad/custom/model-overrides.yaml` on this project, so every layer inherited
the Opus tier). All four layers ran: `blind-hunter`, `edge-case-hunter`,
`verification-gap`, `acceptance-auditor` (armed, `review_mode` = full).
Diff: `5aa8616..HEAD` plus the working tree plus the two untracked QA test
files, 4,971 lines across 41 files, reviewed in full (large-diff chunking
declined). 19 `patch`, 0 `decision-needed`, 14 `defer`, 12 dismissed.*

**Patched (19, all applied this pass; each new or strengthened pin was
demonstrated red against a named mutation, reverted, and the tree confirmed
clean):**

- [x] [Review][Patch] A finite but UNRESOLVABLE `...Ms` edit escaped the panel's change listener uncaught, wedging the panel and destroying an in-progress recording first [`src/host/dev/tuning-panel.ts`]
- [x] [Review][Patch] `DW-93` gap 3: `replayPlayer.start()` never invalidated an in-progress recording, so a "golden" could silently contain replayed input [`src/host/dev/replay-player.ts`, `src/host/boot.ts`]
- [x] [Review][Patch] AC 4's "gravity follows" half was unpinned: no test observed `setGravity()`'s argument through `createMachine()` [`test/pitch-tunable.test.ts`]
- [x] [Review][Patch] Rule 19: AC 1's Export byte-identity assertion cannot fail for a format regression, and its recorded mutation names code that does not exist [`test/tuning-source.test.ts`, `## Verification`]
- [x] [Review][Patch] Rule 19: AC 1's "`plungerSpeedByHoldMs` does not appear" is vacuous - it is a module export, never a `TUNING` property [`test/tuning-panel.test.ts`]
- [x] [Review][Patch] Rule 19: `DW-86`'s recorded "remove the start-tick rebasing" mutation cannot go red - `rebaseTick` is provably the identity under the `startTick !== 0` guard [`## Verification`, `test/replay-round-trip.test.ts`]
- [x] [Review][Patch] The AC names `runReplay()` as the verifier; no test ever fed a `save()` result to it [`test/replay-round-trip.test.ts`]
- [x] [Review][Patch] `DW-86`'s round trip recorded ZERO input transitions, so `injectTransitions()` and the whole transition path were unexercised by the proof [`test/replay-round-trip.test.ts`]
- [x] [Review][Patch] The `DW-86` record workflow was unreachable through the shipped dev hatch: `boot.ts` passed a live snapshot tick to a `start()` that throws unless it is 0, and no `reset()` was exposed [`src/host/boot.ts`]
- [x] [Review][Patch] `docs/feel-test.md` claimed a build-side PASS and claimed to DELIVER a both-renderer-paths run the same file states was never performed; no `## Environment` table [`docs/feel-test.md`, `test/feel-test-docs.test.ts`]
- [x] [Review][Patch] AC 1: nothing asserted the panel's RENDERED value, `source` or `confidence` - deleting both provenance spans left every test green [`test/tuning-panel.test.ts`]
- [x] [Review][Patch] No test tied the panel's ten class names to `public/styles.css`, though `test/entry-html-csp.test.ts` already establishes that pin for the boot panels [`test/tuning-panel.test.ts`]
- [x] [Review][Patch] Two shipped comments still described the REJECTED coil-held hop gate, contradicting the implemented active-rotation gate [`src/sim/table/tuning.ts`, `test/hardware-rule-seam.test.ts`]
- [x] [Review][Patch] `machine.ts`'s hop comment overclaimed a same-tick POSITIONAL effect; `applyPostStep()` mutates velocity only [`src/sim/physics/machine.ts`]
- [x] [Review][Patch] Rule 19: `test/hop-control.test.ts`'s "hopControl = 0 is the exact identity" stayed green with the `hopControl <= 0` guard deleted [`test/hop-control.test.ts`]
- [x] [Review][Patch] Rule 19: `test/pitch-tunable.test.ts`'s "both runs must have actually moved" checked only `Number.isFinite` (0 is finite), and the movement floor was applied to one run only [`test/pitch-tunable.test.ts`]
- [x] [Review][Patch] Rule 14: a literal `U+2014` in this story's own new file, authored as a raw byte in a template literal [`src/host/dev/tuning-panel.ts`]
- [x] [Review][Patch] Cradling's build-side numbers were pinned only by "some number followed by mm" while the other two items' numbers were pinned exactly [`test/feel-test-docs.test.ts`]
- [x] [Review][Patch] Stale doc comments directing the reader to a panel "Record control" and a panel-reachable `reset()`, neither of which exists [`src/host/dev/replay-recorder.ts`, `src/host/boot.ts`]

**Deferred (14, all via the ledger tool - `by=cr`; no `## Deferred from:` bullets, per the runner's standing override):**

- [x] [Review][Defer] `DW-99` -> `escalated owner=burndown`: the I/O matrix's "the panel shows the clamp" row is genuinely unimplemented, but the clamp lives in `machine.ts`, so showing it needs either duplicated clamp logic in `host/**` (AD-16 smell) or a frame-delayed snapshot read; narrowing the row instead is a Rule 5 ask-first product call. To the decision sheet.
- [x] [Review][Defer] `DW-100` (new) -> `escalated owner=burndown`: Rule 3 - no real-runtime DOM test host exists for any `host/**` UI (`vitest.config.ts` is `environment: 'node'`; the panel is tested through a hand-rolled `document` stub). Every fix is out of this stage's reach: jsdom is a forbidden new npm package, and browser-MCP is Rule 7's explicit allocation to the lead's per-story smoke, which the spec schedules and which is still `pending-author`.
- [x] [Review][Defer] `DW-101` (new) -> `wontfix-accepted`: the panel and 31 KB of `tuning.ts` source text ship in the default-path bundle (static imports); `check:size` is 0.838 / 2.750 MB. `reopen_if` recorded.
- [x] [Review][Defer] `DW-102` (new) -> `wontfix-accepted`: `hopControl` is applied unclamped and untested above the shipped default. `reopen_if` recorded.
- [x] [Review][Defer] `DW-103` (new) -> `wontfix-accepted`: `HOP_COOLDOWN_TICKS` / `HOP_TRIGGER_DELTA_SPEED` are tick-rate-coupled literals. NOT an AD-15 two-class violation (see Dismissed); `reopen_if=TICK_HZ changes from 1000`.
- [x] [Review][Defer] `DW-104` (new) -> `wontfix-accepted`: `setCoilEnabled` is invisible to a recording - neither the header nor the `coilPrologue` carries coil-enable state. `reopen_if` recorded.
- [x] [Review][Defer] `occurrence` on `DW-81`, `DW-91`, `DW-92`, `DW-93`, `DW-94`, `DW-95`, `DW-96`, `DW-97` - existing adjudicated root causes re-sighted this pass, appended rather than re-filed. `DW-91` in particular now looks CLOSABLE (QA's `test/hop-machine-step.test.ts` closes it) while the spec frontmatter and `## Auto Run Result` still list it open - flagged for the lead's `ledger_adjudicated` gate.

**Dismissed (12, the notable ones):**

- **AD-15 / AD-3 "hop.ts's three constants violate the two-class rule"** (reported HIGH by `acceptance-auditor`). Read against the spine's actual AD-15 text: it names "hop control" among the **table tunables**, and `hopControl` IS in `tuning.ts` with `source`/`confidence`. AD-15's two classes describe ported solver constants and table tunables; it does not require every numeric literal in `sim/` to live in one of those two files, and module-local authored constants are existing convention in authored `sim/physics` files (`cabinet/index.ts`'s `SUBSTEP_DT_T`, `nudge-impulse.ts`/`plumb-bob.ts`'s `G_SI`). Not an ADR violation. The residual tick-rate coupling is real and is filed as `DW-103`.
- **`loadFlipper()` still reads the bare `TUNING.materials`** (reported by two layers as a missed spot in the materials-threading fix). Its return value is DISCARDED - it is a key-validation call only, and an override changes a material's values, never its keys. Correct as written, and already explained by its own comment.
- **"A recording stamped with panel overrides can never be replayed by `runReplay()`"** (reported HIGH). That is `assertHeaderMatchesLiveEnvironment` working as designed: a recording made under overrides SHOULD be stale against shipped tuning.
- **`sprint-status.yaml` says `review` while the spec frontmatter says `done`.** Normal pipeline state, not a defect.
- Also dismissed: the frontmatter `deferred:` entries lacking `id:` (the ledger is the canonical record and the harvest is complete); `DW-95`'s `in-store-placeholder` footprint token (append-only body; the lead already appended a correction trailer); `reset()` re-entrancy from inside `onFrame` (nothing calls it there); a coil pulse on the final recorded tick (the live run's hash at `durationTicks` excludes it too, so both sides agree); harness duplication across three test files; a prior recording surviving a rejected `start()` (unreachable through the shipped hatch after the `boot.ts` patch); the `Math.random`-only PRNG grep; and `.dw-tuning-panel`'s hardcoded background colour.

**Guardrails re-verified after every patch:** `pnpm check:ad7` **still exits 1**
with `DW-70`, `AD-7`, `bd_trough`, `[true,true,true,true]` and
`[true,true,true,false]` in its output (`src/sim/loop/index.ts`'s pinned
overwrite is byte-unchanged from `5aa8616`); `DW-85`, the `DW-82` `NOTICE`
residual, `DW-79`'s port-body freeze and `DW-72`'s bound were not touched; no
...
- **AC 1 (export format):** pinning test `test/tuning-source.test.ts`. **CORRECTED at code review — the mutation recorded here previously (`emit entry(value, source) without the confidence argument → the byte-identity assertion goes red`) names code that DOES NOT EXIST and was never applicable.** What shipped is not an emitter: `serialiseTuning([])` returns the `?raw` import of `src/sim/table/tuning.ts` verbatim through an early return that never touches `applyOverride`/`replaceEntryValue`/`GROUP_OPEN`/`GROUP_CLOSE`/`ENTRY_OPEN`. So the byte-identity assertion compares that file's text to the same file's text read through `fs` — it cannot fail for any FORMATTING regression, because nothing formats anything, and the Design Notes' claim that this one test "enforces every convention `tuning.ts` actually has … without anyone having to enumerate them" does not hold for the implementation that landed. (It also invalidated `DW-94`'s "currently unreachable" rationale, which rested on that coverage.) The real pins, all demonstrated this pass: `mutation: change replaceEntryValue()'s inline branch to slice from markerIdx instead of markerIdx + marker.length → the single-edit and every-row assertions go red (demonstrated).` Plus a NEW every-row round trip added at review — one override per enumerated panel row must change exactly one line, carrying the new value — which is what extends the guarantee from three hand-picked paths to every leaf Export can actually be asked to emit.
...
- **AC 4 (pitch):** pinning test `test/pitch-tunable.test.ts` -- `defaultPitchDeg` 8.5 vs 6.0, asserting `snapshot.effectivePitchDeg`, a measurable downfield acceleration difference, and a differing `playfield_root` rotation. `mutation: revert machine.ts's effectivePitchDeg derivation to TABLE.reference.pitchDeg → effectivePitchDeg stays 6.5 and six of the file's assertions go red.` **CORRECTED at code review:** the second half of that claim ("and the acceleration-difference assertion goes red") was FALSE. The acceleration block builds its own `loadCollision()` and calls `physics.setGravity(pitchDeg, …)` BY HAND, so it is a property of `PlayerPhysics.setGravity()`, not of this story's derivation — which left AC 4's "gravity … follows" half unpinned against the smaller, realistic mutation. `mutation: change ONLY machine.ts's setGravity() argument to TABLE.reference.pitchDeg, leaving the effectivePitchDeg derivation intact → before this pass, EVERY test in the repo stayed green; a new block added at review (a ball served and stepped through the real public Machine.step() at 6.0 vs 8.5) now goes red (demonstrated: "expected 2.868726968765259 to be greater than 2.868726968765259" — byte-identical downfield velocities).`
...
- **`DW-86` (record→replay round trip):** pinning test `test/replay-round-trip.test.ts` -- record through the real `createHostLoop` on the existing manual-rAF harness, save, replay, assert identical `stateHash`. **CORRECTED at code review — `mutation: remove the start-tick rebasing from save()` CANNOT go red and was not a demonstrated pin.** `start()` throws `NonZeroStartTickError` unless `startTick` is 0 and `capturedStartTick` is assigned only from that argument, so `rebaseTick(t, 0)` is the identity on every legal path (`replay-recorder.ts`'s own doc comment concedes as much) and deleting the rebasing changes no output at all. The genuine pins: `mutation: remove the non-zero-tick guard from start() and begin recording mid-session → the hash-equality assertion goes red` (real, and its own test in the file), plus two gaps closed at review — the test recorded ZERO input transitions (a stripped copy of `test/host-loop.test.ts`'s harness whose `listeners` map was written and never read, so `injectTransitions()` and the transition path were never exercised; a real `ShiftLeft` press/release is now dispatched and asserted non-empty), and nothing ever fed a `save()` result to **`runReplay()`** — the function the AC actually names. `runReplay()` on the saved recording is now asserted to reproduce the live session's own `stateHash`, which is also the first real evidence for the Design Notes' "directly promotable to a golden" claim. It passes.
...
**QA-pass additions (`bmad-qa-generate-e2e-tests`, this stage — closes the residual gap this spec's own frontmatter `deferred:` list names, and confirms one of the "known-weak area" cross-seam findings with real, running evidence):**

- **AC 2 (hop, driven through the REAL public `Machine.step()`, not the hand-wired harness):** new pinning test `test/hop-machine-step.test.ts` (QA) — the same paired `hopControl = 0`-vs-default divergence `test/hop-control.test.ts` already pins, but this time via `createMachine().step()` itself: one ball served through the real trough-eject pulse command (so it is genuinely physics-registered, not merely pushed onto `machine.balls`), repositioned against the left flipper's face, then struck by driving the real step loop with `flipper_l` held. Measured this pass: `hopControl = 0` → max z 13.526 mm (the resting height); default (0.35) → 23.961 mm — a 10.435 mm margin, closely matching `test/hop-control.test.ts`'s own isolated-harness figure (~11.9 mm/strike). `mutation: clamp the hop impulse to 0 unconditionally (hop.ts's own vel.z += hopControl * excess → vel.z += 0) → the divergence assertion goes red (demonstrated: 0 mm divergence vs the 5 mm named margin, i.e. "expected 0 to be greater than 5"), reverted, git hash-object src/sim/physics/hop.ts confirmed byte-identical to before (8948fc35b2e9f103a86585168d45b2932f5edc58).` Closes the spec's own frontmatter `deferred:` residual-gap finding ("no test drives a hard flipper strike through createMachine()'s own public step()") without either alternative that finding named (a purpose-built golden, or a Machine API addition) — it reuses `test/machine-serve-drain.test.ts`'s own established serve-then-reposition technique instead.
- **Cross-seam coordination (confirming, not fixing, the story's own deferred finding — not AC-scoped, so Rule 19's per-AC mutation discipline does not apply; falsifiable on its own terms instead, see the file's own header):** new test `test/dev-seam-coordination.test.ts` (QA) — drives a real `createHostLoop()` + `createReplayRecorder()` + `createReplayPlayer()` through the "Play pressed while Record is still running" scenario the deferred finding describes, and confirms by direct execution (not just code reading) that `replayPlayer.start()` never calls `replayRecorder.invalidate()`: the in-progress recording's `isRecording` stays `true` through playback, and a subsequent `save()` still returns `{ ok: true }` despite the sim having been reset and fed replayed transitions mid-recording. This test will need updating (its `isRecording`/`save().ok` expectations flip) the day a future story wires the missing invalidation — that is the intended, expected way for it to go red.

**Code-review-pass additions (`/bmad-code-review`, 2026-08-30 — each applied,
observed red, reverted, and the working tree confirmed clean before the next):**

- **AC 1 (panel, unresolvable `...Ms` edit):** new case in `test/tuning-panel.test.ts`. `resolveTuning()` → `msToTicks()` (DW-35) throws on a negative `...Ms` duration and on a positive one rounding to 0 ticks; the panel enumerates every `...Ms` leaf and `-1` is ordinary typing in a number input. The throw escaped the `change` listener having ALREADY invalidated a live recording and ALREADY stored the value in `overrides`, so every later edit to ANY row threw too — the panel was wedged until a page reload (no close affordance, `DW-95`). `mutation: remove the try/catch in hotApply() → red (demonstrated: "expected [Function] to not throw ... 'Error: resolveTuning(): \"slamNudgeWin…'").`
- **AC 1 (rendered provenance):** new case asserting the DOM a developer actually sees. `mutation: drop the pathEl/metaEl appendChild calls → red (demonstrated: "expected [] to include 'high — FR-10 consequence text…'").`
- **AC 1 (enumeration, the vacuity):** `plungerSpeedByHoldMs` is a module-level `export function`, never a `TUNING` property, so `rows.some(…) === false` held for every possible implementation and the panel's own `typeof node === 'function'` guard was never executed by it — this epic's FOURTH structurally-unfalsifiable assertion. Replaced by a fixture that drives the behaviour directly. `mutation: delete the function early return AND weaken the object check to node === null → red (demonstrated: ['realLeaf','asFunction.nested'] vs ['realLeaf']).` Recorded honestly: the two guards are mutually redundant, so the `typeof node === 'function'` line is provably dead code and no single-guard mutation can pin it.
- **AC 1 (styles):** new case collecting class names from the BUILT panel DOM and requiring a rule for each in `public/styles.css`, the pin `test/entry-html-csp.test.ts` already applies to the boot panels. `mutation: rename .dw-tuning-panel__heading in public/styles.css → red (demonstrated).` Note the first attempt at this pin was itself vacuous — a bare `toContain('.dw-tuning-panel__status')` passes against `.dw-tuning-panel__statusRENAMED` — so the match is anchored on a non-identifier boundary.
- **AC 2 (the zero guard):** `test/hop-control.test.ts`'s "hopControl = 0 is the exact identity" stayed GREEN with `hopControl <= 0 ||` deleted, because the loop then computes `vel.z += 0 * excess`. A NEGATIVE `hopControl` — reachable, since the panel enumerates it as an ordinary editable row — distinguishes them. `mutation: delete the hopControl <= 0 term → red (demonstrated: "expected -485 to be +0"), while the zero test stays green.`
- **`DW-93` gap 3 (fixed, not deferred):** the ledger's own note called it "a correctness hole inside `DW-86`'s own deliverable … expected fix-now at code review", and Rule 15 makes an in-footprint MED with low fix-risk non-deferrable. `createReplayPlayer()` gained an optional narrowed `replayRecorder` dep and `start()` now invalidates an in-progress recording before resetting the sim — the same contract and ordering as the panel's hot-apply (AD-15). QA's `test/dev-seam-coordination.test.ts` flipped from a characterization test asserting the DEFECT to a regression pin on the fix, exactly as its own header predicted. `mutation: remove the invalidation from replay-player.ts's start() → red (demonstrated: "expected true to be false").`
- **`DW-86` (dev hatch reachability):** `boot.ts` passed the live `latestSnapshot.tick` to a `start()` that throws unless it is 0, and the loop has run since boot — so every console `start()` threw and the record/save/play workflow was unreachable in a real browser, with no `reset()` exposed and no panel Record control (the recorder's own doc comment pointed at one). The facade now resets first and passes a truthful 0; `reset` is exposed on `window.__dragonwarBoot`.
- **AC 5 (`docs/feel-test.md`):** the document simultaneously claimed to deliver "the both-renderer-paths run of the build side", carried a "build-side PASS" verdict, and stated "Both named here, per AC 5; neither yet run". The browser leg is Rule 7's allocation to the lead and had not been run, so the first two were wrong and nothing in CI caught the contradiction. Verdict and intro corrected to name the headless leg only; the missing `## Environment` table (the `docs/spikes/spike-3.md` house format the Code Map cites) added; `test/feel-test-docs.test.ts` gained pins for the environment table, for cradling's own numbers (previously matched only as "some number followed by mm"), and for the verdict not reading as an unqualified PASS.
