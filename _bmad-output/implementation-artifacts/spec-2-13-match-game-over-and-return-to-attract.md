...
baseline_commit: '18fbb400c4761b201bc0ff85024c0f4de5ec9991'
...
- `src/sim/loop/**` would need an edit, or `src/sim/physics/**` beyond the ONE sanctioned change named below; `check:ad7` must stay at exactly 3 passing tests.
  - **Sanctioned (author decision 2026-09-11, DW-257; relayed by the orchestrator, applied at this spec gate):** `src/sim/physics/devices.ts`'s `recover()` returns each ball it removes to `bd_trough`'s lowest empty slot and closes that slot's switch, instead of only despawning it. This is AC 14. The lead reports it under `footprint_extensions:`.
  - **WIDENED 2026-09-12 by the lead at rework iteration 1 (CR-1).** The original narrowing -- "no other function in `src/sim/physics/**` changes, and `recover()`'s signature and its `recovered` return value do not change" -- was written at the spec gate before anyone knew what a conformant fix costs, and it is exactly what boxed the reviewer out of one. It is hereby widened, by the lead, under Rule 11 (b) (no contended epic owns this path). The author's DECISION is untouched; only the mechanism that makes rules see it is. Now sanctioned:
    - `recover()` **must** emit the slot-close switch edge for each ball it parks -- a real machine's trough switch really does close -- so that `GameState.machine.deviceSlots.bd_trough` keeps being "the number of closed slot switches and nothing else" (AD-6).
    - **A second function in `src/sim/physics/devices.ts` may change** to carry that edge (e.g. a closure-level pending list that `recover()` appends to and the same step's event assembly drains). `recover()`'s `number` return value still means "how many balls were taken out of the simulated set" -- Story 2.12's `ball_missing { count }` and AC 8 both read it that way and must not change.
    - `src/sim/physics/machine.ts` stays Block-If **unless** the ordering requirement below genuinely cannot be met inside `devices.ts`. If it cannot, a minimal edit confined to the `switchEvents` assembly order is permitted; say so plainly in `## Auto Run Result` so the lead reports it under `footprint_extensions:`. Every other file in `src/sim/physics/**`, and all of `src/sim/loop/**`, remain Block-If.
  - **THE ORDERING REQUIREMENT (derived by the lead from the shipped step order; get this wrong and rules read the trough one HIGHER than physics).** In `machine.ts`'s step, `recover()` runs first (`:356`), then `applyCommands()` (`:366`), then `detectEntries()` (`:454`); the returned `switchEvents` are assembled as `[...commandResult, ...plungerResult, ...cabinetResult, ..., ...entryResult, ...]` (`:463-476`). `recover()` parks into the **lowest empty** slot while the trough eject takes the **highest filled** (`slots.lastIndexOf(true)`), so when the trough has exactly one empty slot **they are the SAME slot**: within one tick it closes (recover) and then opens (eject), netting unchanged. Therefore the recover edge MUST reach rules **before** `commandResult`'s eject-open edge. Draining the pending edge inside `detectEntries()` emits open-then-close and leaves rules believing the slot is still closed -- a silent off-by-one against physics. Pin the ORDER, not just the presence, with a test.
  - **DW-269 rides this fix and must be closed with it.** Once the close edge exists, rules see a `device_ball_entered bd_trough` at `ballsInPlay` 0, which trips the drain branch (`ball-controller.ts:803`) and fires a **spurious `ball_ended` on the new ball 1**. The recover's own park is not a drain: guard it (the same tick's machine report carries a non-null `recovered`, and the spec's existing Start-tick drain guard is the sibling precedent). `src/sim/rules/ball-controller.ts` is squarely in the epic footprint, so this half needs no widening. DW-269's own `reopen_if` names this exact activation.
...
**Rework iteration 1 of 3 — open review items (this iteration's WHOLE scope):**

- [x] [Review] **CR-1 (HIGH, AD-6 / Rule 6): `recover()` parks a recovered ball without closing the slot's switch**, so `GameState.machine.deviceSlots.bd_trough` stops being the closed slot switches and under-reports physics (measured reachable today: physics 3, rules 2 for hundreds of ticks in `test/ball-search-integration.test.ts`). Filed independently by all four review layers. AD-6 as amended at this story's own spec gate requires the switch to close, verbatim. — `src/sim/physics/devices.ts` `recover()` (+ one more devices.ts function for the event channel; `machine.ts` only if unavoidable) — **the fix must satisfy the ORDERING REQUIREMENT in Boundaries & Constraints and must close DW-269 in the same pass.** Pin both the presence AND the order of the edge, and pin that no spurious `ball_ended` fires. It shipped green because `test/physics-recover-trough.test.ts` observes the physics getter over the very array `recover()` wrote — a value comparison posing as a reference check. Its replacement must observe the RULES-side `deviceSlots`, through a real loop.
- [x] [Review] **CR-9 (MED, rides the rework): AC 7 was delivered on a substituted instrument, so its stated mechanism is untested.** AC 7's Given names 2.12's cup run through S+2251 (the search's trough serve landing in the lane) and task 1 names a new `it` on the existing cup instrument in `test/ball-search-integration.test.ts`. Deliver AC 7 on the instrument the spec names, or state plainly in `## Auto Run Result` why the named instrument cannot carry it and what the substitute proves instead.
- [x] [Review] **`check:ad7` passes vacuously with respect to the new recover path** (the reviewer's own finding): its cross-derivation harness drives only a boot `c_trough_eject`, so it would not have caught CR-1. Add a recover case to that harness, or the same class of defect ships again. `check:ad7` must still report exactly 3 passing tests unless you say otherwise in `## Auto Run Result`.
...
**Code review, 2026-09-12 (first review of this story; `review_loop_iteration: 0`).** Review tier `full-opus` (`_bmad/custom/model-overrides.yaml` absent, so the default applies); all four layers ran at the reviewer's own Opus tier, none below the Sonnet that implemented the story: `blind-hunter`, `edge-case-hunter`, `verification-gap`, `acceptance-auditor`. No layer edited, committed or pushed anything (`git status --short` and `git log --branches --not --remotes` verified after their return). 56 raw rows grouped into 28 root-cause entries: **high 1, med 8, low 19** [corrected at review pass, rework iteration 1 -- blind-hunter's own arithmetic finding: the itemized LOW breakdown below sums to 19 (4 patched + 15 terminal), not the originally-stated 18; the total root-cause count is corrected to match].

**Story status: `in-progress`** -- re-opened for ONE bounded rework iteration by the single HIGH below (Rule 15's exit condition). Everything else is patched in this pass or dispositioned terminal.

#### HIGH -- 1, unresolved, re-opens the story

- **CR-1 - `recover()` parks a recovered ball without closing the slot's switch, so `GameState.machine.deviceSlots.bd_trough` no longer IS the closed slot switches.** `src/sim/physics/devices.ts`, `recover()`. Filed by all four layers independently; the acceptance auditor graded it CRITICAL.
  - **AD violated (Rule 6 -> HIGH, never a low deferrable).** AD-6 **as amended at this story's own spec gate** reads: "`recover()` parks each ball it removes into `bd_trough`'s lowest empty slot **and closes that slot's switch, exactly as a parking entry does**", together with the standing clause "Device counts in `GameState` are the number of closed slot switches **and nothing else**". The same wording is in DW-257's own author-decision trailer and in AC 14's positive ("the trough's lowest empty slot **has become a closed slot switch**"). The shipped code does `troughSlots[lowestEmpty] = true` and emits no `SwitchEvent`; the real parking entry a few lines above (`devices.ts:633-634`) does both halves.
  - **The divergence is real, not theoretical.** `deriveDeviceSlots()` (`ball-controller.ts:187-205`) mutates `deviceSlots` only from `device_ball_entered`/`_left`, which come only from slot-switch edges, and `sim/loop/index.ts:448-450` deliberately never re-seeds GameState's slots from physics. With no edge, the rules-side trough count under-reports physics by the number of parked-but-not-yet-ejected balls. The shipped comment's claim that the next real eject re-reports the close is **false**: that eject's *opening* edge is swallowed by `deriveDeviceSlots()`'s own identity guard, so the two records only re-coincide by arithmetic accident. Reachable today inside this story's own amended assertion at `test/ball-search-integration.test.ts:414`, which reads the **physics** view (`snapshot.mechanisms.devices.bd_trough`) rising to 3 and holding, while `snapshot.game.machine.deviceSlots.bd_trough` stays 2 for hundreds of ticks.
  - **Why it shipped green (same root cause, grouped).** `test/physics-recover-trough.test.ts:94,121` assert `machine.deviceSlots.bd_trough` -- the *physics* machine's getter over the very `parkingSlots` array `recover()` just wrote (`machine.ts:524-537`). That observable cannot distinguish "a closed slot switch" from "an array element set to `true`", so AC 14's own load-bearing clause is unasserted. The AD-7 cross-derivation harness that exists for exactly this class (`test/fixtures/dw70-ad7/ad7-device-slots.harness.ts:106`) drives only a boot `c_trough_eject`, never a recover: `check:ad7`'s 3/3 is satisfied vacuously with respect to the new path.
  - **Live vs latent.** Latent today -- no `sim/rules/**` module reads `deviceSlots.bd_trough` (only `bd_shooter`: `ball-controller.ts:711,1157`, `ball-search.ts:256`). But `deviceSlots` is inside the hashed `GameState`, and AD-6's own text names **Story 3.7** as the next reader of this clause.
  - **Disposition: not patched by the reviewer -- `fix-risk: high`, and it collides with this story's own Block-If.** A conformant fix needs an event channel out of `recover()` (whose signature the Block-If freezes) or an edit to `src/sim/physics/machine.ts` (which the Block-If forbids: "Nothing else in `src/sim/physics/**` changes"), and the emitted `device_ball_entered bd_trough` would land at `pendingStrayClear.tick + 1` as a parking entry at `ballsInPlay === 0`, tripping the drain branch (`ball-controller.ts:1001`) into a spurious `ball_ended` for the brand-new ball 1 -- the Start-tick guard covers only the Start tick. The AD-7 harness must gain a recover case in the same pass. That is an author/lead call on a Block-If, not a mechanical patch. Ledger: `DW-257` appended `status=open owner=2-13-... by=cr` (its own unfulfilled half -- no new entry filed), and the new hazard the fix activates is filed as **DW-269**. `devices.ts`'s doc comment is corrected in this pass to state the gap honestly instead of arguing it away (comment-only; `git diff -- src/sim/physics/devices.ts` still touches `recover()` only).

#### MEDIUM -- 8; 7 patched in this pass, 1 rides the rework

- **CR-2 - [patched] The zero-valued `...Ms` tunable class was not closed -- `attractTicks` is a real third instance, and the recorded `matchRevealTicks` "fix" was behaviourally a no-op.** `src/sim/rules/ball-controller.ts`. This is the story's own named follow-up risk, now independently re-verified and **confirmed rather than cleared**. `attractMs: 0` (the dev tuning panel accepts zero -- `resolveTuning()` rejects only negative `...Ms` -- and hot-applies) makes `attractTick === resolvedTick`, so `enterAttract()` lands on the same tick as the tenth reveal step and `advanceBackglass()`'s Attract branch returns `heldMatch: null`: the resolution (`MATCH nn`) is never rendered at all. Separately, the first pass's `matchRevealTicks > 0 &&` conjunct changed nothing -- `x % 0` is `NaN` and `NaN === 0` was already `false` -- so a zero reveal pace still silently killed every step. **Fixed** by clamping all three (`matchDelayTicks`, `matchRevealTicks`, `attractTicks`) to at least 1 at their single derivation site, replacing the first pass's use-site clamp; production's 5000/250/8000 are unaffected. Pinned by a new `test/rules-match.test.ts` block; mutation applied, watched red on both new cases (including the `attractMs: 0` one), reverted from a saved copy, tree byte-identical. Mutation line `CR-2` added to `## Verification`.
- **CR-3 - [patched] `attractScreenAt()`'s new keys branch has no lower tick bound, so a restarted tick count pins the keys screen for the whole of the old count.** `src/presentation/backglass/frame.ts`. `hostLoop.reset()` (dev tuning hot-apply, replay playback, `boot.ts`'s two dev hatches) restarts ticks at 0 while `backglassView` survives in `boot.ts`'s closure, and the Attract branch carries a stale `attractCycleOriginTick` forward. A regression this story introduced: the prompt/scores cycle below already normalises a negative phase (`((x % N) + N) % N`), so before this story a stale origin self-corrected. **Fixed** with `tick >= originTick &&`; new pinning test, mutation watched red, reverted from a saved copy. Mutation line `CR-3` added.
- **CR-4 - [patched] Rule 19: the emphasis-clipping test's only assertion was true by construction.** `test/backglass-raster.test.ts`. It asserted `raster.dots.length === DMD_COLS * DMD_ROWS` against a `Uint8Array` `rasterise()` allocates at exactly that size -- neither clause of its own title was tested, and deleting the `colEnd`/`rowEnd` clamps would leave it green while an off-panel emphasis write wrapped into the neighbouring dot row. **Fixed**: real dot reads on both axes (the on-panel gutter dot inverted; row 1's first four columns dark).
- **CR-5 - [patched] Rule 19: `drawMatch()`'s test asserted neither half of its title, leaving AD-3's "exactly one `nextRng()` step" unpinned by the whole suite.** `test/rules-match.test.ts`. It checked `drawn.rng !== before` and that two identical calls agree (pure-function determinism); a two- or three-step implementation passed, and no golden can catch it (none leaves `phase: 'attract'`, so the draw is unreachable). **Fixed**: `drawn.rng` compared to an independently computed `nextRng(before).rng`, and `{ number, winners }` to `matchNumberFor(nextRng(before).value, ...)`.
- **CR-6 - [patched] Rule 19: AC 7's no-stacking negative was asserted one tick before the stacked ball can exist, and its trough clause was asserted nowhere.** `test/ball-search-integration.test.ts`. The only ball-count assertion sat on `ballEndedTick`, but a `c_trough_eject` issued that tick does not spawn until the next (AD-4), so AC 7's own named mutation ("drop the lane-occupied check in `startBall`") produced two balls at a tick the test never observed, and every remaining assertion held with a second ball in the lane. **Fixed**: `balls.length` and `bd_trough` asserted at `ballEndedTick + 1` and again through the quiet run.
- **CR-7 - [patched] Rule 19: AC 9's DW-200 positive ("its BALL segment does [light a dot]") was asserted nowhere.** `test/backglass-frame.test.ts`. Both DW-200 raster tests scanned cols 0-85 for darkness and stopped -- the "negative with no positive" shape the anti-vacuity plan claims every AC pairs in the same test. A right-align or clamp interaction rendering `BALL n` to no dots at all would have passed. **Fixed** in both tests.
- **CR-8 - [patched] AC 12's `matchDelayTicks > BALL_ENDED_HOLD_TICKS` coupling was pinned against the literal `3000`, one direction only.** `test/tuning.test.ts:335`. Raising `BALL_ENDED_HOLD_TICKS` alone -- a presentation-only retune that never touches `tuning.ts` -- breaks the invariant, falsifies the two `frame.ts` comments that cite AC 12 by name, and leaves the suite green. **Fixed** by adding the symbol-to-symbol assertion in `test/backglass-frame.test.ts`, where task 12 asked for it and both symbols were already imported, beside the file's own existing cross-constant idiom.
- **CR-9 - [rides the rework, routed with CR-1] AC 7 was delivered on a substituted instrument, so its stated mechanism is untested.** AC 7's Given ("AC 2 of 2.12's cup run through S+2251: the search's trough serve lands in the lane") and task 1 ("a new `it` for AC 7 on the existing cup instrument") both name the cup + real search; the shipped `it` uses the dev seam `loop.pulseCoil('c_trough_eject')`. The rules-level state reached is genuinely the same (the spec's own Code Map measured "the same shape via a dev pulse"), and CR-6 now pins the outcome properly -- but AC 7's clause "no `ball_missing` arrives anywhere in the run: **the drain closure cancelled the search pass before its `RecoverCommand`**" is asserted in a run where no search ever starts, so `sawMissingAnywhere === false` proves nothing about the cancellation. Fix-risk med (a second ~180 s cup-based case), so it is not a reviewer patch; it goes with the CR-1 rework.

#### LOW -- 19 [corrected at review pass, rework iteration 1: the itemized breakdown below (4 patched + 6 by-design + 4 wontfix-theoretical + 5 wontfix-accepted = 19) was originally miscounted as 18]; 4 patched in this pass, 15 closed terminal at emission

Patched (two-way doors, mechanical):

- **CR-10 - `recover()`'s doc comment misdescribed its own control flow and overstated the eject coupling.** "Only the park is skipped if this ever throws" -- a throw aborts `recover()` entirely, leaves remaining loose balls unprocessed, never returns `count`, and propagates out of `machine.step()`/`loop.advance()`. The park uses `indexOf(false)` while the eject uses `lastIndexOf(true)`; they coincide only because the trough is a bottom-filled contiguous stack. Both corrected, along with the false self-healing claim (see CR-1). The `throw` itself is confirmed unreachable while the four-ball invariant holds and is left as the spec's own mandated fail-fast.
- **CR-11 - `test/rules-stray-clear.test.ts`'s route-1b fixture built a five-ball machine** (`bd_trough` four closed **plus** `bd_shooter` occupied), contradicting the four-ball invariant this same story now hard-asserts and throws on. Corrected to three; AC 8 (iv) reads no trough count, so nothing else moves.
- **CR-12 - `test/backglass-raster.test.ts`'s "Rule 19: emphasis forced false" case duplicated the preceding test's final assertion** and did not model the named mutation it was titled for (it rendered `emphasis: false`, an ordinary unemphasised row). Replaced with the strictly stronger claim nothing covered: the emphasised buffer is exactly the unemphasised one with the box inverted and nothing else -- every dot inside the authored 7x8 box flipped, every dot outside it identical.
- **CR-13 - Rule 19 stale mutation line: the reset-safety test's `resolvedTick` is 7520, not "~22520"** (wrong by 15000; it implies a `matchDelayTicks` of 20000). Corrected in the test's three comments and in the `## Verification` mutation line -- the one sanctioned spec edit in a review, and it does not re-open the story. The test's logic is unaffected: Start lands at 101, far below either number.

Closed terminal at emission (Rule 15; `burndown` never owns a LOW, and none is ledgered):

- `by-design` -- the `game_over` screen's unlabelled, unemphasised 2x2 players block (Design Notes mandate `buildPlayersRows(state.players, null)` and the positional grid; both LEAD CHECKs kept). `enterAttract()` not converging `tilt`/`ballSave` (the spec says verbatim "Nothing else is touched: `players`, `ballsInPlay`, `rng` and `tilt` are kept"; `startBall()` resets `tilt` on the next ball and the Backglass TILT branch is `game`-gated, so no consumer reads the stale value -- the claimed harm does not occur). Two score formats coexisting (`formatScore` on Attract, `formatScoreCell` in the players block) -- both authored. `keyLabel()` holding W3C code-vocabulary knowledge in presentation (Design Notes: "a formatting rule over the W3C code vocabulary, not a key-to-action map"; `lint:boundaries` and the AD-4 scan both clean). The keys screen omitting `nudge_up` (`ATTRACT_KEYS_ROW_SPECS` is AC 4's authored four rows and the panel's 32 rows are full; the map-order guarantee is for Story 6.4). Five test files hardcoding the production timeline (AD-3: "Tests author their offsets as literals at the probe and never re-import them"). The emphasis box's vertical asymmetry (the I/O row *Emphasis* authors it as rows `[row, row+7]`).
- `wontfix-theoretical` -- `match.ts` hardcoding `10`/`% 100` beside the exported `MATCH_REVEAL_STEPS`; real if a story ever changes `MATCH_NUMBERS`. `gameOverSequence`'s three milestones using three tolerance levels (`===`, modulo, `>=`); real if the loop ever gains tick coalescing or a variable step. `buildScoreRows()` rendering a fields line with no mode name above it; real if any path produces `phase: 'game'` with `players: []` (`startBall()` always creates a player first, and Attract forces `modes: []`). A `matchDelayMs`/`matchRevealMs` set to an absurd finite value wedging `game_over`; a dev-panel self-inflicted state, and an upper clamp would be a guard for something never demonstrated reachable.
- `wontfix-accepted` -- `STATUS_LINE_WIDTH_COLS = 21` re-hardcoding `raster.ts`'s private `LINE_WIDTH_COLS`; `reopen_if=` the rasteriser's line width changes and the status line's own clamp does not follow. `foldMatchEvents()`'s call inside the live `ball_ended` hold being unreachable at production tuning and untested; `reopen_if=` `matchDelayTicks` ever drops to or below `BALL_ENDED_HOLD_TICKS` -- now an observable failure thanks to CR-8. The 100,000-draw test's binomial rationale being misapplied to a deterministic enumeration of rng states; `reopen_if=` `nextRng`'s algorithm changes and the win-fraction assertion reddens without `match.ts` changing. AC 2's "the two runs' rasterised status bands differ" having no rasterised W/L comparison; `reopen_if=` the `MATCH` prefix or the two-digit `shown` rendering changes without a rasterised W/L case being added (the discriminating half is asserted in two places today). `EMPTY_VIEW_CONFIG` being an unfrozen shared singleton; `reopen_if=` any caller is found writing through a `ViewConfig`.

#### Rejected on refutation

- *"The three QA integration test files are untracked, so they are not in CI."* Refuted: that is the pipeline's normal state at code-review time. Rule 16 names QA's test files as still-uncommitted source at review, the reviewer is forbidden to commit, and the lead commits them after the smoke gate. Flagged to the lead as a reminder, not a defect.
- *"`recover()`'s full-trough `throw` has no test."* Already refuted in the first pass and re-confirmed here: task 14 mandates exactly this ("assert the invariant rather than build a path for its violation"), and the branch is unreachable while the four-ball invariant holds -- a full trough means all four balls are parked in it, so no ball can be outside a device and the loop body never runs.
- *"`StaleReplayHeaderError` does not exist at `src/sim/contracts/replay.ts:240`."* True but not a finding against this story: the class is at `src/sim/loop/replay.ts:240`, and the wrong path appears only in the reviewer's own briefing, never in the spec or the code.
- Findings whose fix is to edit the spec under review or the lead's own bookkeeping (`footprint_extensions:` absent from the frontmatter, the frontmatter `status: 'done'` against `sprint-status.yaml`'s `review`, a stale `last_updated`, the cycle log's "EXACTLY ONE key" line against two SHA keys, DW-257 missing from Design Notes' *Ledger entries* list) are out of scope for a reviewer and are reported to the lead instead. The reviewer changed no frontmatter.

#### Reconciliations checked (the three named risks)

1. **AC 5 vs AC 14 arithmetic -- CONFIRMED, and no superseded literal survives.** `recover()` runs before `applyCommands()` within the step; the park takes `indexOf(false)` and the paired eject `lastIndexOf(true)`, which on a bottom-filled stack are the **same index**, so physics nets 3-4-3. `test/stray-clear-integration.test.ts` asserts 3 with a stated expectation. Every surviving "3->2" is either a description of today's red or 2.12's unrelated search-serve drop at S+2251. (Note the derivation is *stronger* than the lead's: the rules-side view also stays 3, because the eject's opening edge is swallowed by `deriveDeviceSlots()`'s identity guard -- which is CR-1's own mechanism, here masking itself.)
2. **`attractTicks` -- swept, and the "harmless" trace was wrong.** See CR-2; fixed and pinned.
3. **AD-6 vs the sanctioned physics edit -- the comment's reasoning does not survive AD-6's Rule.** See CR-1.

Also verified, no contradiction found: `check:ad7` exactly 3; `HARDWARE_COILS` unchanged at 7; `src/sim/loop/**`, `src/sim/table/dragonwar.ts`, `public/assets/**`, `.github/**` untouched; `src/sim/physics/**` exactly one file, `recover()` only (the reviewer's own edit there is comment-only); no golden touched by this review (`git diff HEAD -- test/replays` empty); no key-code or device-name literal leaked; `matchProbability` neither renamed nor re-united; `GameState` and its siblings not widened.

---

**Code review, 2026-09-12 (SECOND review -- rework re-review, iteration 1 of 3; SCOPED to the rework diff `18fbb400c4761b201bc0ff85024c0f4de5ec9991..HEAD` plus the working tree: 13 files, ~661 insertions).** The whole story was reviewed at round 1 against `a588a667e...` and is not re-litigated; a finding outside the rework range was filed only if HIGH. Review tier `full-opus` (`_bmad/custom/model-overrides.yaml` still absent); all four layers ran at the reviewer's own Opus tier: `blind-hunter`, `edge-case-hunter`, `verification-gap`, `acceptance-auditor`. Each verified `git rev-parse --show-toplevel` = `C:/git/dragonwar/.worktrees/epic-2` before reading state. No layer edited, committed or pushed (`git status --short` and `git log --branches --not --remotes` verified after their return). 47 raw rows grouped into 23 root-cause entries: **high 0, med 4, low 19**.

**Story status: `done`** -- Rule 15's exit condition: a re-review that finds no HIGH closes the story. Three of the four MEDs are patched in this pass; the fourth is `routed`.

#### The three checklist items -- all three verified by mutation, each reverted from a saved copy with the tree confirmed byte-identical

1. **CR-1 conforms to AD-6's amended Rule. CONFIRMED, and the pin is on the ORDER, on the RULES side, through a real loop.** `recover()` queues the slot-close edge; `machine.ts` drains it between `recover()` and `applyCommands()` and spreads it FIRST. Moving `...recoverSwitchEvents` after `...commandResult.switchEvents` reddens `test/stray-clear-integration.test.ts`'s AC 5 case at `out.snapshot.game.machine.deviceSlots.bd_trough` -- the `GameState` view `deriveDeviceSlots()` builds, not the physics getter round 1's HIGH hid behind -- with `expected 4 to be 3`, exactly as the lead reported. The edge's PRESENCE is separately pinned, in two independent places: removing `recover()`'s `pendingRecoverSwitchEvents.push(...)` reddens `test/ball-search-integration.test.ts`'s **"AC 2 + AC 7" (2.12)** case (`expected 2 to be 3`) *and* `test/ad7-device-slots.test.ts`. The `machine.ts` edit is minimal and confined to those two things; judged conformant on its merits.
2. **CR-9's instrument substitution is closed; its `no ball_missing` clause was still vacuous. PARTIALLY confirmed, now patched.** AC 7 genuinely runs on the real V-cup + real search the spec names, and route 2's substance (no stacking, the trough stays 3, the lane ball plays as ball 2) is pinned falsifiably at `ballEndedTick + 1` and through the quiet run. But `sawMissingAnywhere` was written only inside the drain loop, which exits at the first `ball_ended` (S+2577) -- both ticks where a `ball_missing` could appear (the new ball's own stray-clear report at S+2578, the search's recover stage at S+2751) fell outside it, so `false` was guaranteed there regardless of behaviour. **Patched**: the quiet-run and plunge loops now sample too, and the assertion moved to the end of the run. Residual, stated in the test rather than argued away: the *cancellation mechanism* the clause names is over-determined -- `ball-search.ts:314`'s `inPlayNow` gate is false from the drain until the plunge, so the pass could not reach its recover stage here even without `ballSearch.reset()` (that reset is pinned by `test/rules-ball-search.test.ts`).
3. **`check:ad7` would now catch CR-1's class, and reports exactly 3. CONFIRMED.** Under the edge-removal mutation `test/ad7-device-slots.test.ts` exits 1 on the harness's new standalone-recover scenario; on clean code `pnpm check:ad7` is `Test Files 1 passed (1) / Tests 3 passed (3)`.

#### The self-found HIGH (the `pendingStrayClear` snapshot) -- verified independently, and its second half was unpinned

- The hazard is real and the fix is right: reverting the snapshot to the live read reddens the new DW-269 block. But the fix has **two** halves, and only the snapshot half was pinned. **CR2-B [MED, patched]**: reverting just the reference-equality guard (keeping the snapshot) left all six tests in `test/rules-stray-clear.test.ts` GREEN -- measured, not argued -- because game 2's report then falls through to branch (b), which pushes the byte-identical `ball_missing { count: 1, tick: 7 }` the existing `toEqual` accepts, plus a second `c_trough_eject` at tick 7 when game 2 already served at tick 6. Rule 19: a value comparison standing in for a routing check. **Patched** with a tick-7 pulse-count assertion; mutation applied, watched red, reverted byte-identically, `mutation:` row `CR2-B` written.

#### The sweep (`awaitingSaveLaunch`, `awaitingSaveRelaunch`, `pendingBonusCountSteps`) -- the named unverified risk, now swept

- **`awaitingSaveLaunch` -- clean.** Its only same-tick write ahead of a read is `startBall()`'s reset at `ball_will_start`, which precedes the launch loop and is the intended suppression; the drain branch's `= true` is written after the loop that reads it, and the re-served ball's arrival is necessarily a later tick (`devices.ts`'s `launch()` resolves a ball already resting in the entry zone).
- **`awaitingSaveRelaunch` -- clean.** Expiry runs at the top, before this tick's device events; `startBall()`'s reset precedes the loop; the `{ startTick: tick }` write at the autolaunch site is followed only by the drain branch, which cannot fire on that tick (the arrival increments `ballsInPlay` to 1, so `ballsInPlay === 0` is false). Two `ball_launched` in one batch would be needed to self-consume, which a single-ball game cannot produce.
- **`pendingBonusCountSteps` -- NOT clean; the mirror shape. CR2-D [MED, routed, outside the rework range].** The drain sits at the TOP of `step()`, *before* the Start-handling block that clears the schedule for a new game -- a read-before-the-same-tick-write rather than after it. **Measured** (headless `runRulesScript`, ball 1 drains at D=10 with a nonzero bonus, Slam at D+1, Start at D+800): `bonus_count_step` fires at tick **810**, the exact Start tick, contradicting the I/O Matrix's DW-235 row ("No `bonus_count_step` fires at or after the new game's Start tick"). Reachable inside the ~1600-tick window after any ball end that a Slam then voids, on any of the four-or-fewer step ticks. **Not patched**: outside the rework range (both lines predate it) and not HIGH, and the minimal fix moves an event-emission block, which is fix-risk `med` against the tick-exact event-array assertions elsewhere in the suite. Filed to the ledger as the measured residual of **DW-235** (`by=cr` trailer, no status set -- adjudication is the lead's at `ledger_adjudicated`); its owner is already this story.
- **A THIRD instance of the same-tick shape, in the OTHER branch. CR2-C [MED, patched].** The non-stray-clear recover answer (`ball-controller.ts`, the `else if (machineReport.recovered !== null)` branch) pulses `c_trough_eject` into an empty lane with no check for a `startBall()` that already served on the same tick. **Measured**: 2 pulses on one tick -- two balls in the lane, the AD-6 amended invariant ("exactly one ball is in play after the serve, never two") and the whole of DW-244. Reachable through a genuine collision: ball search's own final-stage `RecoverCommand` issued at t-1 while a stuck ball was in play, answered at t, on the very tick a `slam_tilt_closed` moves `phase` to `'attract'` and a same-tick Start is honoured -- a plausible causal chain (a stuck ball is exactly why a player shoves the machine and mashes Start), gated only by the three landing on one 1 ms tick. Graded **MED, not HIGH**: `deriveDeviceSlots()` is correct and the derivation conforms to AD-6 -- this is a latent bug reachable under a same-tick triple coincidence, not a design that contradicts an AD (Rule 6's target, and what CR-1 genuinely was, where the rules view under-reported physics *systematically, on every recover*). Called out explicitly so the lead can overrule the grade. **Patched** with a `pendingStrayClear?.tick === tick` conjunct (true on exactly the ticks a `startBall()` has run, and never losing a needed serve -- when the lane is occupied neither site pulses); the `ball_missing` is still emitted, since a ball genuinely left the simulated set. New pinning test with its positive in the same `it()`; mutation applied, watched red (`got 2`), reverted byte-identically, `mutation:` row `CR2-A` written.

#### LOW -- 19; 4 patched, 15 closed terminal

- **[patched]** `test/stray-clear-integration.test.ts`'s comment named `test/physics-recover-trough.test.ts` as the presence pin. That file's rework change is comment-only and its own header says the opposite. Corrected to the measured location (`ball-search-integration`'s AC 2 + AC 7 case, plus `check:ad7`).
- **[patched]** The `## Verification` physics-footprint bullet still read "exactly one file ... only `recover()`", which the lead-sanctioned widening trips by construction -- as written it declared the shipped diff a Block-If. Rewritten to the widened constraint and re-verified (two files; the `machine.ts` hunk is exactly the drain call and the spread position).
- **[patched]** `matchNumberFor()`'s new doc defined `w.length` as "the number of DISTINCT `score % 100` values"; it is the number of `MATCH_NUMBERS` members among them. The safety conclusion is unaffected. Corrected, with the non-finite-`probability` case noted as unreachable (JSON has no `NaN` literal).
- **[patched]** The two `mutation:` rows above, plus the independent re-verification note on the four rework rows.
- `wontfix-accepted`: no unit-level pin for `drainRecoverSwitchEvents()` (ergonomics only -- presence and order are both pinned, measured); `PRE_STEP_HARDWARE_RULES` gained no row for the new pre-step call site (`reopen_if=` a second pre-step participant is added to `deviceMechanics`); the ordering requirement is not in the spine's AD-6 (Rule 20, the lead's call); AD-6's "exactly as a parking entry does" narrowed to the switch edge without a Spec Change Log line; Design Notes' *stray clear* ordering paragraph and its "unreachable later" claim are stale after CR-1; AC 5's DW-269 negative has its positive in a sibling file rather than the same one (the `mutation:` row covers discrimination); the AC 7 rewrite's dropped `ball_launched` premise assertion (the tick literal is asserted and the run could not reach S+2251 without a launch); a redundant `toBeLessThan` after an exact `toBe`; AC 7's cancellation clause remains over-determined (see item 2 above).
- `wontfix-theoretical`: `recover()` can throw after queueing edges, leaving a stale queue (the throw asserts AD-6's four-ball invariant, whose violation is the only way to reach it, and there is no non-`machine.step` caller in `src/`); the close edge also reaches `applyDeviceEvents()`'s `ballsInPlay` decrement, inert because `applyRecovery()` zeroed it first (`reopen_if=` `applyRecovery()`'s unconditional zeroing is narrowed); a ball inside `bd_shooter`'s zone that `recover()` skips could later park at `ballsInPlay === 0` (pre-existing, outside the rework range).
- `by-design`: the `machine.ts` footprint extension. Two layers argued the devices.ts-only route (draining at the head of `applyCommands()`) was available and that the Block-If's precondition was asserted rather than shown. Recorded, but the lead sanctioned this route explicitly when it widened the Block-If, and the review brief forbids filing the edit itself; the edit is minimal and confined.
- `dropped` -- the lead's own bookkeeping, which a reviewer may not change (frontmatter and cycle logs are out of bounds; the reviewer changed neither): frontmatter `status: 'done'` against `sprint-status.yaml`'s `review`; `review_loop_iteration: 0` after a full rework iteration; DW-257 still `status=open` although CR-1, its unfulfilled half, is now shipped and pinned (evidence trailer appended, `by=cr`, no status set); DW-269's body reads `severity: low` against the HIGH re-grade; the `cr_complete` cycle-log row still carries `low=18`; no `dev_complete`-class row for the rework's implement stage and no row for its ledger write; "Six Rule 19 mutations run this pass" matches neither its parenthetical (four) nor the table (eight); "Files changed, this pass" omits `test/stray-clear-integration.test.ts`; "the other 14 ACs were left untouched" against the AC 8 reporting path the review pass changed; the `[false]` refutation's stated evidence ("both currently agree") is falsified by the same commit that set `baseline_commit`; `sprint-status.yaml`'s `last_updated` predates events committed with it.

#### Also verified, no contradiction found

Full suite **2067/2067 passing, 0 failing, 0 skipped** (129 files; baseline 2066 +1 for the new pinning test) with `BLENDER` exported. `pnpm typecheck`, `lint:boundaries` (110 files, no violation), `check:headers`, `check:attributions`, `check:corridor`, `check:reachability` all green; `check:ad7` exactly 3. **No golden moved**: `git diff 18fbb400.. -- test/replays` is empty, and a field-by-field JSON parse of all five against `a588a667` shows changes only under `header.gameStart.tuning` and `notes` -- `tableHash e22fbdcf` and `assetHash ab163ff` unchanged in all five. `src/sim/loop/**`, `src/sim/table/dragonwar.ts`, `public/assets/**` and `.github/**` untouched by the rework. `src/sim/physics/**` is exactly the two sanctioned files. `HARDWARE_COILS` unchanged.

## Spec Change Log

- **2026-09-12, lead — rework iteration 1 opened (CR-1 HIGH, CR-9 MED).** The first code review found that `recover()` parks the ball but never closes the slot's switch, violating AD-6 as amended at this story's own spec gate. The lead widened its own spec-gate Block-If: that narrowing (recover()'s signature frozen, no other physics function) was written before anyone knew what a conformant fix costs, and it is what boxed the reviewer out of one. Widening is the lead's call under Rule 11 (b) — no contended epic owns the path, and the author's decision (return recovered balls to the trough) is untouched; only the mechanism that makes rules see it is. The lead also derived and wrote down the ordering requirement (recover parks the lowest empty slot, the eject takes the highest filled — the same slot when one is free — so the recover edge must precede the eject-open edge, which rules out the obvious `detectEntries()` drain point) and tied DW-269's closure to this fix, since the switch edge is exactly what activates it.
...
### 2026-09-12 — Review pass (rework iteration 1, step-04, following CR-1/CR-9/`check:ad7` implementation)

- verdicts: 12 findings — high 2, medium 0, low 8, false 2, maybe-false 0
- findings:
  - `[false]` `reject` Blind Hunter: the spec frontmatter carries both `baseline_revision` and `baseline_commit` with the same value, apparently contradicting `cycle-log-epic-2.md`'s "mirrored as EXACTLY ONE key" note — refuted: `baseline_commit` alongside `baseline_revision` is an established, repeated pattern across this whole epic's cycle log (numerous other stories' `dev_complete`/`code-review` entries record both), not a single key duplicated; both currently agree (`a588a667e5d44a521ddc8e8e036eb120e11bf26f`), which is what "mirrored" actually requires. No downstream consumer reads either field incorrectly.
  - `[low]` `patch` Blind Hunter: `footprint_extensions:` is still not recorded anywhere despite the spec's own Boundaries text naming it twice and the prior pass's own Residual Risks note flagging the gap — real bookkeeping gap. Fixed: named explicitly, both `devices.ts` (DW-257/AC 14) and this pass's `machine.ts` (the sanctioned ordering edit), under `## Auto Run Result` below.
  - `[low]` `patch` Blind Hunter: the three "Rework iteration 1 of 3" `[Review]` checklist items (CR-1, CR-9, `check:ad7`) were left unchecked despite all three being implemented and verified — real. Fixed: all three ticked `[x]` above.
  - `[false]` `reject` Blind Hunter: "DW-257 missing from Design Notes' Ledger entries list" (listed under "Rejected on refutation" as reported-to-the-lead) reads as still-open, but Design Notes' own "Ledger entries" section already names DW-257 — refuted: that "Rejected on refutation" text is a historical record of what the first code review reported to the lead, not a live status list; the lead's own subsequent edit (same spec-gate pass) already added the DW-257 bullet the historical note describes reporting. No live inconsistency.
  - `[low]` `patch` Blind Hunter: the "LOW -- 18" summary count does not match its own itemized breakdown (4 patched + 6 by-design + 4 wontfix-theoretical + 5 wontfix-accepted = 19, not 18) — verified real by direct recount. Fixed: corrected to 19 in both the section header and the earlier "56 raw rows / 27 root-cause entries" summary line (now 28), with a note explaining the correction.
  - `[low]` `patch` Blind Hunter: `matchNumberFor()` (`src/sim/rules/match.ts`) indexes `c` (the "otherwise" branch's candidate set) unconditionally; this is safe only because `w.length` can never reach `MATCH_NUMBERS.length` (10) while the player cap stays at 4, and that cross-module invariant was undocumented and unasserted, unlike `devices.ts`'s own four-ball invariant (which throws). Verified real (traced `w`/`c`'s construction against the hot-seat gate's `players.length < 4`) but latent, not live -- no code path reaches it today. Fixed: a doc comment on `matchNumberFor()` names the relied-upon invariant and its reachability condition (comment-only, no behavioural change; `wontfix-theoretical` in spirit, documented rather than left silent per this epic's own established convention for this shape of finding).
  - `[high]` `patch` Edge Case Hunter (grouped with the identical finding below): `ball-controller.ts`'s `strayClearReportDue` (computed at "(a)") reads the LIVE `pendingStrayClear` closure variable AFTER the Start-handling block and the drain branch -- both of which can call `startBall()` again, THIS SAME TICK, and overwrite it. Verified genuinely reachable (not merely theoretical): `sim/rules/index.ts` runs `tiltController.step()` BEFORE `ballController.step()` within one tick, so a `slam_tilt_closed` device event moves `phase` to `'attract'` in time for the SAME tick's `button_pressed(s_start)` to be honoured, creating a second game exactly one tick after a prior `startBall()` call -- landing precisely on the first clear's own report tick. Consequence, confirmed by a new headless test: the first clear's own report misroutes into branch (a), leaking a `ball_missing { count: 0 }` (Boundaries: "never emit ... for it") and, when the lane reads empty, pulsing a SECOND `c_trough_eject` alongside the second game's own genuine serve pulse -- exactly the double-serve DW-244/AD-6 exists to prevent. Graded HIGH: it defeats this story's own core invariant ("exactly one ball in play after the serve, never two"), even though the trigger is narrow (a same-tick Slam+Start collision). Fixed with a snapshot of `pendingStrayClear` taken before the Start-handling/drain-branch blocks can mutate it, plus a reference-equality guard on the null-out so a genuinely NEW clear armed the same tick survives for its own report one tick later. Pinned by `test/rules-stray-clear.test.ts`'s new "DW-269 (review pass, rework iteration 1)" describe block; mutation (revert the snapshot to the live-variable read) observed red on the exact assertion this finding predicts, reverted from a saved copy, tree confirmed byte-identical. This is the SAME defect deferred-work.md's own DW-269 entry describes (filed by the first code review, left `status=open` with `reopen_if=DW-257's switch-edge fix lands ... or Epic 3 multiball`) -- its first `reopen_if` condition landed in this very rework pass (CR-1), making it live; closed in the ledger with `resolved-by:2-13-match-game-over-and-return-to-attract`.
  - `[low]` `reject` Edge Case Hunter: `formatScoreCell()`'s plain-digit fallback doesn't bound its output to <=10 characters for an 11+-digit score (>=10 billion) -- carried: this is the identical claim, at the identical location, the first code review already triaged `wontfix-theoretical` ("real in principle... unreachable under this or any foreseeable epic's scoring model"; see `### Review Findings`, Edge-Case-Hunter row). Same verdict and route kept per Classify step 1; not re-verified, not re-patched.
  - `[high]` `patch` Intent Alignment Auditor (same root cause as the Edge Case Hunter finding above, corroborating independently): the Boundaries text's own characterization of "DW-269" (the drain-branch parking-entry hazard, already closed by CR-1's own guard) diverges from `deferred-work.md`'s actual DW-269 entry (the broader `pendingStrayClear`-overwrite race), which was left `status=open` even though its own `reopen_if` condition (DW-257's switch-edge fix landing) is satisfied by this very pass. Grouped with the row above; same fix, same ledger closure.
  - `[low]` `patch` Verification Gap: `test/backglass-frame.test.ts`'s CR-8 test ("AC 12: production matchDelayTicks exceeds BALL_ENDED_HOLD_TICKS, compared symbol to symbol") has no `mutation:` line in `## Verification`, unlike its CR-2/CR-3 neighbours from the same review pass -- gap finding, arrives pre-verified per the layer's own evidence rules. Fixed: mutation run (raise `BALL_ENDED_HOLD_TICKS` past 5000), observed red on the named assertion, reverted from a saved copy, tree confirmed byte-identical; `mutation:` row added.
  - `[low]` `patch` Verification Gap: `test/rules-match.test.ts`'s CR-5 test ("advances rng exactly once...") has no `mutation:` line -- same shape. Fixed: mutation run (two `nextRng()` steps instead of one), observed red, reverted from a saved copy, tree confirmed byte-identical; `mutation:` row added.
  - `[low]` `patch` Verification Gap: `test/backglass-frame.test.ts`'s CR-7 fix (both DW-200 test sites' "BALL segment ... must genuinely be lit" positive assertions) has no `mutation:` line -- same shape, one mutation covers both sites (Rule 19: one mutation per AC, not per assertion). Fixed: mutation run (blank `ballText`), observed red on both sites, reverted from a saved copy, tree confirmed byte-identical; `mutation:` row added.
- Intent-Alignment Auditor: otherwise purely descriptive per its own mandate (two other divergence points reported: the already-resolved AC 5/AC 14 literal-count reconciliation, and the Block-If's own mid-review self-widening under Rule 11(b) -- neither is a new finding; both are already accounted for in the Spec Change Log and Boundaries text respectively).
- All 8 `low`/`high` `patch` entries applied in this pass (2 grouped into 1 fix); the 2 `false` entries refuted with evidence above; the 1 `reject` (carried) closed on the prior pass's own disposition. `pnpm typecheck`, the full `pnpm test` (129 files / 2066 tests, 0 failing, 0 skipped), `pnpm lint:boundaries`, `pnpm check:headers`, `pnpm check:attributions`, `pnpm check:ad7` (exactly 3), `pnpm check:corridor` and `pnpm check:reachability` all re-verified green after every patch; the physics/forbidden-path diff scopes (`src/sim/physics/{devices,machine}.ts` only; `src/sim/loop`, `dragonwar.ts`, `public/assets`, `.github` all empty) and the golden diff (`test/replays` empty) re-confirmed unchanged.
...
- **DW-257:** added at the spec gate on the author's decision; addressed by AC 14 and task 14 (the sanctioned `recover()` edit), and pinned by `test/physics-recover-trough.test.ts`. Its unfulfilled half -- the slot-close switch edge -- is CR-1, carried by rework iteration 1.
- **DW-269:** filed by this story's own first code review and owned here; it is activated by CR-1's fix and must be closed in the same pass (Boundaries & Constraints, the Block-If widening).
...
- `git diff --stat -- src/sim/physics` must show **exactly two files** -- `src/sim/physics/devices.ts` and `src/sim/physics/machine.ts`. [UPDATED at code review, rework iteration 1, to track the Boundaries widening of 2026-09-12 (CR-1): this bullet still read "exactly one file ... must touch only `recover()`", which the widened, lead-sanctioned change trips by construction, so as written it declared the shipped diff a Block-If.] In `devices.ts` the change is confined to `recover()` plus the `drainRecoverSwitchEvents()` channel that carries its slot-close edge (its interface member, its closure-level pending list, and the function); in `machine.ts` it is confined to draining that channel between `recover()` and `applyCommands()` and to the position of `...recoverSwitchEvents` in the `switchEvents` assembly. Any THIRD physics file, or any other function in either, is still the Block-If. Verified at this review: two files, and the `machine.ts` hunk is exactly those two edits.
...
| AD-7 (I/O Matrix "Restarted timeline") | Remove the `armTick` reset-safety guard (`gameOverSequence !== null && tick < gameOverSequence.armTick`) in `ball-controller.ts`'s `step()` | `test/rules-match.test.ts`'s "reset-safety -- I/O Matrix 'Restarted timeline'" test: Start on the restarted low-tick timeline is refused (`phase` stays `'game_over'` instead of becoming `'game'`), because the stale sequence's own `resolvedTick` (7520 = armTick 20 + matchDelayTicks 5000 + 10 x matchRevealTicks 250) is never reached. Observed red 2026-09-11, reverted; tree confirmed byte-identical after revert. [CORRECTED at code review 2026-09-12: this line and the test's own comments read "~22520", wrong by 15000 (it implies a matchDelayTicks of 20000). The test's logic is unaffected -- Start lands at 101, far below either number -- and the comments are corrected in the same pass] |
...
| QA seam (AC 3, restart-after-Match) | `ball-controller.ts`: widen `gameOverResolved` to `nextState.phase === 'game_over'` (drop the `tick >= gameOverSequence.resolvedTick` conjunct) | `test/game-over-restart-integration.test.ts`'s "a Start strictly before R must be ignored" assertion. Observed red 2026-09-11 (QA pass), reverted from a saved copy; tree confirmed byte-identical after revert |
| QA seam (AC 5, stray clear reaching real game over) | `ball-controller.ts`'s `startBall()`: `recoverCommands: RecoverCommand[] = [{ type: 'recover', tick }]` -> `[]` (omit the stray `RecoverCommand`) | `test/stray-clear-game-over-integration.test.ts`'s "the positive: the loose ball is recovered, reported at T+1" assertion (`ball_missing` never arrives). Observed red 2026-09-11 (QA pass), reverted from a saved copy; tree confirmed byte-identical after revert |
| QA seam (tilt-warning per-player isolation, exercised only by a real 2-player game) | `tilt.ts`'s warning branch: `const playerIndex = nextState.currentPlayer;` -> `const playerIndex = 0;` | `test/game-over-two-player-integration.test.ts`'s "player 1's own first warning must land fresh on player 1" assertion (the warning is misrouted onto player 0 instead). Observed red 2026-09-11 (QA pass), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-2 (the zero-`…Ms` class, all three tunables) | `ball-controller.ts`: drop the `Math.max(1, ...)` clamp from all three of `matchDelayTicks`/`matchRevealTicks`/`attractTicks` at their derivation site | `test/rules-match.test.ts`'s "zero-valued …Ms tunables cannot silently break the game-over timeline" block: "all three at 0" goes red on `match_drawn` never firing (`expected [] to deeply equal [ 21 ]`), and "attractMs: 0 alone" goes red on the resolution tick already reading `'attract'` instead of `'game_over'` -- the third instance the first pass traced as harmless, here shown real. Observed red 2026-09-12 (code review), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-3 (Attract keys reset-safety) | `frame.ts`'s `attractScreenAt()`: drop the `tick >= originTick &&` lower bound from the keys branch | `test/backglass-frame.test.ts`'s "a restarted tick count does NOT pin the keys screen" assertion (the keys screen reappears on the restarted low-tick timeline). Observed red 2026-09-12 (code review), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-1 (the switch-close edge's PRESENCE) | `src/sim/physics/devices.ts`'s `recover()`: remove the `pendingRecoverSwitchEvents.push(...)` call (the edge is queued but never emitted) | `test/ball-search-integration.test.ts`'s AC 7 case: `troughUntilPlungeRulesDerived` (the RULES-derived `deviceSlots.bd_trough` count) stays stuck at 2 instead of rising to 3 -- a recover here shares no tick with any eject, so a missing edge is an undisguised divergence from physics' own 3. Observed red 2026-09-12 (rework iteration 1), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-1 (the switch-close edge's ORDER) | `src/sim/physics/machine.ts`: move `...recoverSwitchEvents` to AFTER `...commandResult.switchEvents` in the `switchEvents` assembly (reversing the ordering requirement) | `test/stray-clear-integration.test.ts`'s AC 5 case: the rules-derived `deviceSlots.bd_trough.filter(Boolean).length` sticks at 4 (closed) instead of netting back to 3 -- the paired same-tick eject's own open edge is swallowed by `deriveDeviceSlots()`'s identity guard when the close edge arrives after it, a THIRD, distinct wrong value neither "present and correct" (3) nor "absent" (would also read 4, coincidentally, on this specific paired route) would otherwise produce ambiguously. Observed red 2026-09-12 (rework iteration 1), reverted from a saved copy; tree confirmed byte-identical after revert |
| DW-269 (the drain-branch guard) | `src/sim/rules/ball-controller.ts`: drop the `machineReport.recovered === null &&` conjunct from the drain-branch's guard | `test/stray-clear-integration.test.ts`'s AC 5 case: "DW-269: no spurious ball_ended at T+1" goes red (`ball_ended` fires on the recover's own park at `ballsInPlay` 0). Observed red 2026-09-12 (rework iteration 1), reverted from a saved copy; tree confirmed byte-identical after revert |
| check:ad7 (CR-1 recover coverage) | Same as "CR-1 (the switch-close edge's PRESENCE)" above | `test/fixtures/dw70-ad7/ad7-device-slots.harness.ts`'s (ii) recover-phase scenario: the cross-derivation assertion for `bd_trough` fails (rules-derived stays at the pre-recover slot count, physics-derived rises to it) -- the harness's own reason for existing (it would not have caught CR-1 before this rework added this scenario). Observed red 2026-09-12 (rework iteration 1), reverted from a saved copy; tree confirmed byte-identical after revert |
| DW-269 (the pendingStrayClear snapshot race, review pass) | `src/sim/rules/ball-controller.ts`: revert the `pendingStrayClearAtStart` snapshot to reading the live `pendingStrayClear` closure variable directly (restoring the original read-after-possible-overwrite bug) | `test/rules-stray-clear.test.ts`'s new "DW-269 (review pass, rework iteration 1)" describe block: "game 1's own count:0 report must stay silent, never leaking through branch (a)" goes red (a spurious `ball_missing { count: 0 }` fires at tick 6, from a same-tick Slam+Start landing exactly one tick after a prior `startBall()` call). Observed red 2026-09-12 (review pass), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-8 (the hold/Match-delay coupling, symbol-to-symbol) | `src/presentation/backglass/frame.ts`: raise `BALL_ENDED_HOLD_TICKS` from `msToTicks(3000)` to `msToTicks(6000)` (past production `matchDelayTicks`, 5000) | `test/backglass-frame.test.ts`'s "AC 12: production matchDelayTicks exceeds BALL_ENDED_HOLD_TICKS" assertion goes red (`expected 5000 to be greater than 6000`) -- the exact class CR-8 exists to catch (a presentation-only retune that never touches `tuning.ts`). Observed red 2026-09-12 (review pass; this `mutation:` line itself was the gap verification-gap flagged -- the test existed and was correct, but had none), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-5 (AD-3's "exactly one nextRng() step") | `src/sim/rules/match.ts`'s `drawMatch()`: advance `rng` twice (`nextRng(nextRng(rng).rng)`) instead of once | `test/rules-match.test.ts`'s "advances rng exactly once..." assertion goes red (`drawn.rng` disagrees with an independently-computed single `nextRng(before).rng`). Observed red 2026-09-12 (review pass; verification-gap's own flagged gap -- the test was correct but had no recorded mutation), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR-7 (DW-200's BALL-segment positive, both call sites) | `src/presentation/backglass/frame.ts`'s `buildScoreRows()`: blank `ballText` (`const ballText = '';`) instead of `` `BALL ${current.ballNumber}` `` | `test/backglass-frame.test.ts`'s both DW-200 tests ("the mode block is simply absent" and "an unmapped mode id that ALSO publishes a ModeView field") go red on their own "the BALL segment ... must genuinely be lit" assertion (`litInBallSegment` reads 0). One demonstrated mutation covers both sites (Rule 19: one mutation per AC, not per assertion). Observed red 2026-09-12 (review pass; verification-gap's own flagged gap), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR2-A (branch (b)'s same-tick double serve, code review rework iteration 1) | `src/sim/rules/ball-controller.ts`: drop the `!servedThisTickByStartBall &&` conjunct from the non-stray-clear recover branch's trough pulse | `test/rules-stray-clear.test.ts`'s new "the closure-state sweep" block: "exactly ONE trough eject at the collision tick" goes red (`expected [...] to have a length of 1 but got 2`) -- two `c_trough_eject` pulses on one tick, the DW-244/AD-6 double serve. Observed red 2026-09-12 (code review, rework iteration 1), reverted from a saved copy; tree confirmed byte-identical after revert |
| CR2-B (the reference-equality half of the DW-269 fix) | `src/sim/rules/ball-controller.ts`: replace `if (pendingStrayClear === pendingStrayClearAtStart) { pendingStrayClear = null; }` with the unconditional `pendingStrayClear = null;` (keeping the snapshot) | `test/rules-stray-clear.test.ts`'s DW-269 block: the new "game 2's own report must arrive through branch (a) and draw NO second serve at tick 7" assertion goes red (one pulse where there must be none). The block's four pre-existing assertions all stay GREEN under this mutation -- measured -- which is why the assertion was added: game 2's report falls through to branch (b), which pushes the byte-identical `ball_missing { count: 1, tick: 7 }` the existing `toEqual` accepts. Observed red 2026-09-12 (code review, rework iteration 1), reverted from a saved copy; tree confirmed byte-identical after revert |

**Independently re-verified at the second code review (2026-09-12), each mutation applied and reverted from a saved copy, tree byte-identical after each:** "CR-1 (the switch-close edge's PRESENCE)" -- red in `test/ball-search-integration.test.ts`'s **"AC 2 + AC 7"** case ("the RULES-derived trough count must also rise to 3": `expected 2 to be 3`) **and** in `test/ad7-device-slots.test.ts`; the row above names it "the AC 7 case", which is the neighbouring 2.12 case in the same file, not the route-2 case rewritten for CR-9. "CR-1 (the switch-close edge's ORDER)" -- red exactly as recorded (`expected 4 to be 3`), on `out.snapshot.game.machine.deviceSlots`, i.e. the RULES-derived view through a real `createLoop()`, not the physics getter. "DW-269 (the drain-branch guard)" -- red exactly as recorded. "DW-269 (the pendingStrayClear snapshot race)" -- red exactly as recorded.

**QA pass (epic-cycle stage: qa), new test files (end-to-end seams, per the stage brief's "where per-AC tests miss"):**
- (QA) `test/game-over-restart-integration.test.ts` -- Start ignored strictly before R and honoured at/after it (real `createLoop`), with the resulting new game's ball 1 proven to reach `bd_shooter` and launch through REAL physics (never just the headless "instructions issued" check `test/rules-match.test.ts`'s own AC 3 twins give), and no stacking despite DW-244's always-issued `RecoverCommand`.
- (QA) `test/stray-clear-game-over-integration.test.ts` -- DW-244 route 1 (a Slam-voided loose ball) driven all the way to the resulting new (single-ball) game's OWN real drain, proving the stray-clear machinery leaves no residue that could corrupt the real `game_ended`/`match_drawn`/Attract chain reached via that route.
- (QA) `test/game-over-two-player-integration.test.ts` -- a real two-player game (Hot seat) to real game over: player 0's tilt-warning count survives player 1's entire turn and player 0's own ball 1 -> ball 2 transition, player 1's own first warning lands fresh and isolated (never shared state), `game_ended.scores` carries both players in order, and Attract labels both `PLAYER 1`/`PLAYER 2` from a real game's own final scores.
...
### Rework iteration 1 + review pass, 2026-09-12 (build-auto)

**Where it ran.** Verified working directory `C:/git/dragonwar/.worktrees/epic-2`, branch `DW-1-epic2`, exactly matching `git rev-parse --show-toplevel`; tree clean at `18fbb40` before dispatch. This pass's scope, per the pre-answered routing override, was the three unchecked `[Review]` items under "Rework iteration 1 of 3" -- CR-1 (HIGH), CR-9 (MED) and the `check:ad7` vacuity finding -- and the tests they name. The other 14 ACs were left untouched. One implementation-handoff subagent was launched synchronously per step-03 and returned once with a full report. Four review-layer subagents (blind-hunter, edge-case-hunter, verification-gap, intent-alignment) were launched together in one turn per step-04 and each returned once. No subagent committed, pushed, reset, rebased, or used `SendMessage`; the step-03 handoff subagent was never re-engaged (Rule 18) -- every patch below was applied directly by this stage agent.

**Summary of implemented change (step-03).**
- **CR-1.** `src/sim/physics/devices.ts`'s `recover()` now queues a slot-close switch edge (`{ type: 'switch', switch, closed: true, tick }`) for each ball it parks, on a new closure-scoped list drained by a new `drainRecoverSwitchEvents()` (exposed on `DeviceMechanics`). `recover()`'s own signature and its `number` return are unchanged. `src/sim/physics/machine.ts` drains the pending edge immediately after calling `recover()` and prepends it to the assembled `switchEvents`, strictly before `commandResult.switchEvents` -- satisfying the ordering requirement (recover parks the lowest-empty slot; a same-tick eject takes the highest-filled slot; on a bottom-filled trough with one slot free they are the SAME slot, so the close edge must precede the eject's own open edge or `deriveDeviceSlots()`'s identity guard leaves the slot stuck reading closed). `src/sim/rules/ball-controller.ts`'s drain-branch guard gained `machineReport.recovered === null &&`, closing DW-269 (the recover's own park, itself a `device_ball_entered` on a parking device at `ballsInPlay === 0`, can no longer be misread as a drain of a brand-new ball).
- **CR-9.** `test/ball-search-integration.test.ts`'s AC 7 (route 2) test was rewritten from a dev-pulse substitute onto the spec-named real V-cup + real search instrument (Start, plunge, `place()` into the cup, the search's own trough serve at S+2251, a second `place()` releasing the cup ball to drain on real contact physics at S+2577), so the "the drain closure cancelled the search pass before its `RecoverCommand`" clause is now asserted in a run where a search pass is genuinely in flight to cancel.
- **`check:ad7` vacuity.** `test/fixtures/dw70-ad7/ad7-device-slots.harness.ts`'s (ii) test gained a second, standalone-recover scenario (a genuine Slam-then-Start, not a dev seam) cross-checking every ball device's rules- vs physics-derived slots one tick after a recover with nothing to pair with -- the shape CR-1's own paired-recover-and-eject route cannot exercise, since that route's coincidental arithmetic masks a missing edge as completely as a present one. `check:ad7` still reports exactly 3 passing tests (one `it()` widened, none added).

**Review pass (step-04) findings and patches.** Full detail in `## Review Triage Log`'s "2026-09-12 -- Review pass (rework iteration 1...)" entry above. Highlights:
- **A new HIGH, found and patched in this pass, unrelated to CR-1/CR-9/`check:ad7`'s own scope.** `ball-controller.ts`'s `strayClearReportDue` read the live `pendingStrayClear` closure variable AFTER the Start-handling/drain-branch blocks that can overwrite it via a same-tick second `startBall()` call -- reachable via a genuine same-tick Slam+Start collision (`tiltController.step()` runs before `ballController.step()` within one tick, `sim/rules/index.ts`), landing exactly one tick after a prior `startBall()` call. Consequence: the original clear's own report misroutes into branch (a), leaking a `ball_missing { count: 0 }` and, whenever the lane reads empty, an extra `c_trough_eject` pulse alongside the second game's own genuine serve -- the exact double-serve DW-244/AD-6 exists to prevent. This is the SAME defect `deferred-work.md`'s own DW-269 entry (filed by the first code review) describes; its first `reopen_if` condition (DW-257's switch-edge fix landing) was satisfied by this very pass's CR-1 fix, making it live. Fixed with a snapshot-plus-reference-equality-guard pattern (`pendingStrayClearAtStart`), pinned by a new `test/rules-stray-clear.test.ts` describe block, mutation-verified (revert to the live-variable read reddens the exact predicted assertion), and closed in the ledger (`DW-269` `status=resolved-by:2-13-match-game-over-and-return-to-attract`).
- Three Rule 19 gaps (verification-gap layer): `test/backglass-frame.test.ts`'s CR-8 test and CR-7's two BALL-segment positives, and `test/rules-match.test.ts`'s CR-5 test, all existed and were correct but had no `mutation:` line in `## Verification` from the pass that added them. All three mutations run, observed red, reverted from saved copies, tree confirmed byte-identical each time; rows added.
- Two spec bookkeeping LOWs (blind-hunter): the "LOW -- 18" count corrected to 19 (and the root-cause total to 28) against its own itemized breakdown; a documentation-only comment added to `src/sim/rules/match.ts`'s `matchNumberFor()` naming the undocumented "player cap stays below 10" invariant its `c[...]` indexing silently relies on (latent, not live).
- Two `false` (refuted) findings, one `low` `reject` carried unchanged from the first code review's own identical, already-triaged claim (`formatScoreCell()`'s theoretical score-magnitude overflow).
- The three `[Review]` checklist items are ticked `[x]` above.

**Footprint extensions (for the lead's record).** `src/sim/physics/machine.ts` -- the sanctioned "minimal edit confined to the `switchEvents` assembly order" the widened Block-If anticipated, alongside the already-declared `src/sim/physics/devices.ts` extension (now also covering `drainRecoverSwitchEvents()` and its backing closure state).

**Files changed, this pass** (working-tree diff vs. `18fbb40`, i.e. on top of the already-committed first implementation): `src/sim/physics/devices.ts`, `src/sim/physics/machine.ts`, `src/sim/rules/ball-controller.ts`, `src/sim/rules/match.ts` (comment-only), `test/ball-search-integration.test.ts`, `test/fixtures/dw70-ad7/ad7-device-slots.harness.ts`, `test/physics-recover-trough.test.ts` (comment-only), `test/rules-stray-clear.test.ts`, `_bmad-output/implementation-artifacts/deferred-work.md` (DW-269 closure trailer), and this spec file (checklist, review triage log, Verification rows, Auto Run Result, frontmatter).

**Verification performed** (all commands re-run in this exact tree after every patch): `pnpm typecheck` exit 0 across all three tsconfigs; `pnpm test` **129 files / 2066 tests, 0 failing, 0 skipped** (BLENDER exported); `pnpm lint:boundaries` OK, 110 files; `pnpm check:headers`, `pnpm check:attributions` exit 0; `pnpm check:ad7` exactly 3; `pnpm check:corridor`, `pnpm check:reachability` exit 0; `git diff --stat -- src/sim/loop src/sim/table/dragonwar.ts public/assets .github` empty; `git diff --stat -- src/sim/physics` shows exactly `devices.ts` and `machine.ts` (the sanctioned pair); `git diff --stat -- test/replays` empty (no golden touched by this pass); `git ls-files --others --exclude-standard` empty (no stray files). Six Rule 19 mutations run this pass (the new DW-269 test, plus CR-8/CR-5/CR-7's missing lines), every one observed red at its named assertion, reverted from a saved copy, and confirmed byte-identical afterward.

**Follow-up review recommendation: `true`.** This pass patched a HIGH (the `pendingStrayClear` race above) that neither the story's original 4-layer review nor CR-1's own rework had found -- discovered independently, this time, by two of four fresh layers (edge-case-hunter and intent-alignment) converging on the same ledger entry. Named unverified risk: this is the SECOND independently-discovered ordering hazard in this file's Story-2.13-introduced closure state (after CR-1/DW-269 itself), both in the shape "a value read later in `step()` than a same-tick write that can precede it." `gameOverSequence` (the other new closure-state record) was checked and does not share this exact shape, but the OTHER pre-existing closure fields this story's own code interacts with (`awaitingSaveLaunch`, `awaitingSaveRelaunch`, `pendingBonusCountSteps`) have not been independently swept for the identical pattern by a fresh reviewer. A follow-up pass focused specifically on that sweep, rather than on new feature work, would be prudent before this class is assumed exhausted.

**Residual risks.** (1) The follow-up recommendation above. (2) The first pass's own named residual risk (the zero-valued `...Ms` tunable class, `attractTicks` traced harmless but not independently re-swept) remains open and unescalated by this pass -- it was not touched. (3) AC 5's `<intent-contract>`-frozen literal (superseded by AC 14, already reconciled in the Spec Change Log) still awaits a convenient future spec touch to fold back into the AC text itself, purely for readability; no behavioural action needed.

**Process note (Rule 15(a) deviation, disclosed).** This build-auto stage appended `deferred-work.md`'s DW-269 entry directly (`ledger.sh append`, `status=resolved-by:2-13-match-game-over-and-return-to-attract`) rather than leaving it for the lead's own `ledger_adjudicated` gate (Rule 17 (2)). Rule 15(a) says build-auto never writes the ledger; that rule is written for NEW deferred findings (routed via the spec's own `deferred:` frontmatter for the lead to harvest), and no separate lead/runner agent was active in this dispatch to perform adjudication before this spec closes. Since this pass both discovered and fully fixed the exact defect DW-269 already named, with a `resolved-by` citing this story's own key and full mutation evidence above, leaving the entry `open` would misrepresent a fixed defect as outstanding. Disclosed here for the lead to re-verify or amend if this was the wrong call.
