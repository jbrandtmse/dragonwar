...
**2026-09-03 -- code review (full, Tier 1). Four layers run as subagents: Blind Hunter, Edge Case Hunter, Verification Gap, Acceptance Auditor.** No HIGH finding. The Acceptance Auditor verified all ten governing ADs (AD-2, 3, 5, 6, 7, 10, 11, 15, 16, 19) independently against their Rule text and found **no AD violation**; the spec's own "No AC contradicts an AD's Rule" claim holds. All 12 tasks verified delivered except task 7's `bat side` sweep axis (below). AC 4 (Integration) verified at the consumer's own tier: every driven parameter comes from `shotCase(id)` and `driveShot()` has no exported free-coordinate form.

Counts: 11 patched, 5 deferred, 7 dismissed, 0 decision-needed.

**Patched (applied at review; each behavioural gate demonstrated red-then-reverted, tree byte-identical after each):**

- [x] [Review][Patch] `plunge-full`'s dead-witness guard could be satisfied by the serve alone -- its `expectedSwitch` was `s_shooter_lane`, which is `bd_shooter`'s ENTRY switch (`bd_trough.servesInto: 's_shooter_lane'`) and so closes on the trough eject at tick 1, before any plunger hold; AD-6 makes its OPENING the event that means "plunged". Changed to `s_inlane_l`, which only a launched ball that completed the crossing closes. Raised independently by all four layers. [`test/util/reachability.ts`] *mutation: set `plunge-full`'s `plungeHoldTicks` to 0 (never launches) -> `assertWitnessCorpusHealthy()` red ("travelled only 76.31 mm ... expected >= 1500") plus `loop-off-column-left-east-45` red; reverted byte-identical.*
- [x] [Review][Patch] `MIN_WITNESS_SEGMENTS = 500` was structurally unfalsifiable (Rule 19): a segment is pushed every tick regardless of motion, and every witness runs `settleTicks` 320 + `plungeHoldTicks` >= 285 = at least 604 ticks before the plunger can release, so a witness that never launched cleared the floor by construction. Floors raised to 2000 segments / 1500 mm -- between the guaranteed pre-launch count (604) and the measured minimum (4870 segments / 2319.8 mm). [`test/util/reachability.ts`]
- [x] [Review][Patch] No floor on the witness corpus itself: `assertWitnessCorpusHealthy()` iterates `WITNESSES` and passed vacuously over an empty table, while `closestApproachOverAll()` returns `Infinity` for one -- confirming every `unreachable` verdict for the wrong reason. Added `MIN_WITNESSES = 10`, asserted first. [`test/util/reachability.ts`]
- [x] [Review][Patch] The bypass gate exempted **any** line matching a `return driveShot(` shape, not only `driveCase()`'s -- so a second wrapper bypassed the manifest with the gate green, which the test's own title never claimed. Exemption narrowed to the one exact call site, and the total count of `driveShot(` mentions pinned at two (declaration + that call). [`test/shot-reachability.test.ts`] *mutation: added `function driveAnywhere(mm) { return driveShot(mm, 2200, 0, 9000); }` -> red naming line 304; reverted byte-identical.*
- [x] [Review][Patch] Nothing asserted the **inverse** of the bypass guarantee -- that a manifest entry is actually DRIVEN. Deleting a `driveCase()` call left this file, the case-count floor, the evaluation count and the dense sweep all green, so the manifest could drift into certifying release points no routing test exercises: the direct inverse of AC 4. Added a source scan covering literal and templated call sites. [`test/shot-reachability.test.ts`] *mutation: repointed `driveCase('lock-lane-long')` -> red naming `lock-lane-long`; reverted byte-identical.*
- [x] [Review][Patch] The evaluation-count assertion could not fail from the condition its own title named (Rule 19): both sides derived from `SHOT_CASES`, so an emptied manifest reduced it to comparing 0 with 0 -- green. The red the implementer recorded for that mutation came from the separate `MIN_SHOT_CASES` floor, never from here. Replaced with an evaluated-id **Set** compared against the manifest ids, plus an independent floor at `MIN_SHOT_CASES`. [`test/shot-reachability.test.ts`] *mutation: truncated the per-case iteration source to five entries -> red naming every unmeasured case; reverted byte-identical.*
- [x] [Review][Patch] The reachability metric is **plan-only** -- `replayWitness()` discards z, so `closestApproachMm()` compares x/y alone -- with nothing holding release points at the height that makes plan distance the right measure. An elevated (ramp-height) release point would be certified reachable by a witness rolling *underneath* it: a teleport-shaped hole, and Story 2.1f, the named first consumer, is about the Ramp. Added a per-case assertion that every release point sits at the ball-rest height, naming the limitation in the failure message. [`test/shot-reachability.test.ts`] *mutation: set `ramp-return-geometry`'s z to 60 -> red; reverted byte-identical.*
- [x] [Review][Patch] The dense sweep's `MIN_RELEASES_EVALUATED = 300` counted recipes that produced a settle segment, not releases that launched (a segment is pushed every tick, so a plunge that never fired still returned 800+ and counted), and a single total could not see an **axis** being lost: deleting the fine 240-380 plunge loop -- the axis that discovered `plunge-medium-285`, this story's own headline finding -- left 442 releases over the floor with every verdict unchanged, silently degrading the sweep into a replay of the in-suite witness set. Added a per-release path floor and per-axis floors (>= 50 distinct plunge strengths, >= 60 distinct flip ticks; measured 60 and 70). [`test/fixtures/reachability/reachability-sweep.harness.ts`] *mutation: deleted the fine plunge loop -> red in 6 ms ("only 38 distinct plunge strengths, below the floor of 50"), before the sweep even ran; reverted byte-identical.*
- [x] [Review][Patch] The sweep computed a strictly better closest approach than the in-suite gate and **discarded it**, comparing only the reachable/unreachable boolean. Measured deltas are large -- `descend-sling-l` recorded 51.860 mm, swept **24.716 mm** (27.1 mm apart); `descend-ramp-wall-l` 68.450 vs 55.803; `descend-ramp-return-rail` 25.987 vs 16.381 -- and DW-138's evidence quotes the narrow figures as "the measured closest approach". The sweep now prints the recorded value and the delta on every row. Deliberately a report, not an assertion: `closestApproachMm` is *defined* as the in-suite measurement, which is what the in-suite agreement band checks against. [`test/fixtures/reachability/reachability-sweep.harness.ts`]
- [x] [Review][Patch] AC 5's agreement band (0.01 mm) sits exactly on AD-15's own golden position-quantisation grain, with a workable window of only [0.01, 0.027] over ~4000 accumulated ticks. The file's own comment identifies the **sign** as the real signal ("the ball funnels along WHICHEVER surface bounds it ... the SIGN is what moves") while asserting magnitude. Added explicit sign assertions (right margin > 0 clears, left < 0 contacts) alongside the band -- falsified by the same 20 mm mutation, and immune to cross-host float drift. [`test/shot-reachability.test.ts`]
- [x] [Review][Patch] Documentation corrections: the sweep header called `bd_shooter` "the only serve device" (wrong twice -- `bd_trough` serves, and `bd_lock` is a third ball device) and presented its right-bat negative as exhaustive over the machine; `buildSweepRecipes()`'s comment said "240-320 window" against a 240-380 loop, citing a fallback its own stated window excludes; task 7's "stated budget" was never stated; `AGENTS.md` said "a few hundred physics trajectories, tens of seconds" against a measured 471 / ~75 s and gave no trigger for when to run a check that is in neither CI nor typecheck; and the manifest's top-level `nodeBboxMm('col_wall_lane')` now fails four importing files opaquely at import -- the shape task 6 was told to avoid -- and is now wrapped in a self-explaining error. [`reachability-sweep.harness.ts`, `AGENTS.md`, `test/util/shot-cases.ts`]

**Deferred (ledgered at review per Rule 15; entries born `by=cr`):**

- [x] [Review][Defer] **The headline question, answered -- DW-138 (three trailers appended, not a new entry).** Requiring the nine synthetic descending-drop columns to be "reachable" **is** a category error in the gate's *criterion*, but the answer is **not** to exempt them: exempting re-creates exactly the blind spot this story closed, and the spec's own DW-126 argument ("a bevel guarding a face no ball can reach is a bevel guarding nothing") is sound. A drop column asks "can a ball come to rest on this body's top face"; the gate asks "did any ball's centre pass within a ball radius in plan, in any direction, at any speed". That mismatch cuts both ways -- the nine "unreachable" verdicts are weak evidence about geometry, and the three drop columns marked *reachable* (`descend-dragon-n`, `descend-loop-top-west`/`-east`) are certified by fast balls passing through, which does not establish a slow descending arrival either. **Recommendation: redirect the obligation rather than remove it** -- prove a witness reaches the `col_` **body** the probe is about (its `footprintMm` inflated by the ball radius, via `distanceToPolygonMm()`, already imported), so a bevel on an unreachable body still fails, naming the body. DW-138 therefore carries **two** root causes, not one: (i) this criterion mismatch, which no amount of wider searching will fix; (ii) genuinely unsearched origin axes, which is the `top-lane-1/2/3` + `pop-bumper-1` half. -- deferred, DW-138
- [x] [Review][Defer] **The negative was not searched as exhaustively as it is asserted to have been** (the substance of DW-138(ii)). Three implemented, never-teleporting origin axes were never swept: **(a)** `side: 'r'` itself -- task 7's own wording names "bat side" as a sweep axis and `WitnessRecipe` already carries `side: 'l' | 'r'`, but `ReleaseRecipe` has no `side` field and only the left bat is ever driven; **(b)** a **second flip** -- `ReleaseRecipe` admits at most one, so no left-then-right chain, the ordinary way a player moves a ball across the table; **(c)** `bd_lock` / `pulse c_mouth` -- AD-6 gives the Lock an authored eject pose that *is* the Mouth "aimed at the flippers", present in the committed collision document at **(170, 650)** dir (0, -1, 0), with slot zones `sw_lock_1..3` (x 150-190, y 630-678), generic park/eject in `src/sim/physics/devices.ts`, and the outcome already modelled in `test/shot-routing.test.ts` (`Terminal = 'locked'`). That is a second ball origin in the middle of the table, within ~50 mm of `pop-bumper-1`, `descend-dragon-leg-l` and `descend-dragon-leg-r`. Axis (a) is the cheapest and highest-information: one nested loop, and if the top lanes stay unreached the verdict is geometry rather than technique. Header claims corrected at review; the search itself is the next owner's work. -- deferred, DW-138
- [x] [Review][Defer] A `reachable` case records no closest approach, so the baseline is pinned to +/-0.5 mm for the 14 unreachable cases and only to the bare 13.495 mm tolerance for the 25 reachable ones -- `dragon-body` could degrade from 0.610 mm to 13.4 mm with the gate green. AC 2's wording is nonetheless met (it requires failure when a reachable case is *not* reached, which holds) and the Spec Change Log records the measured approaches, so this is precision, not direction. [`test/util/shot-cases.ts`] -- deferred, **DW-139** (`wontfix-accepted`; `reopen_if=any reachable case's live closest approach is ever measured above half REACHABILITY_TOLERANCE_MM`)
- [x] [Review][Defer] `check:reachability` -- the only instrument that can falsify an `unreachable` verdict -- runs in no CI job, has no in-suite wrapper, and is excluded from typecheck (`tsconfig.node.json` excludes `test/fixtures/**`), so renaming `MIN_SHOT_CASES` or reshaping `ShotCase.startMm` breaks it while typecheck, `pnpm test` and CI all stay green. The no-wrapper decision is spec-bound (Design Notes rejected it deliberately); the CI and typecheck gaps are not, and both are out-of-footprint. Mitigated at review by documenting the run triggers in `AGENTS.md`. [`.github/workflows/ci.yml`, `tsconfig.node.json`] -- deferred, **DW-140** (`escalated`)
- [x] [Review][Defer] The DW-130 record runs on `driveCaseSwept()`, a hand-copied third implementation of `driveShot()`'s stepping loop with no equality check against it and no `assertReleaseClear()`, and its case set includes `ramp-return-geometry` -- a release point this same story proves unreachable. The recorded +0.0198 mm minimum comes from `left-loop-orbit-28`, not from it, so nothing is wrong today; a geometry change could move the minimum onto a trajectory no real ball can produce. [`test/shot-reachability.test.ts`] -- deferred, **DW-141** (`escalated`)

**Dismissed (7):** spec frontmatter `status: done` vs `sprint-status.yaml` `review` (the normal build-auto -> review handoff, not a defect); "39 cases vs 36 distinct release points" (both figures correct, both stated in the Code Map); recorded-value precision inconsistency, 2 dp vs 3 dp (the agreement band is 0.5 mm); cycle-log `files=6` (lead's artifact, cosmetic); the `ledger:` field being validated as a format so `DW-999` would pass (coupling a test to BMAD bookkeeping is a worse seam than the hole it closes); `test/reachability-harness-integrity.test.ts` being untracked (the lead commits it; it carries the sanctioned header); the sweep's self-overlapping recipes (deliberate insurance, 9 of 471).

**Re-verified after all patches:** `pnpm typecheck` clean (three projects) -- `npx vitest run test/shot-routing.test.ts test/shot-reachability.test.ts test/reachability-harness-integrity.test.ts` **187 passed** (38 + 128 + 21; +40 this pass) -- `npx vitest run` (full suite) **90 files / 1348 passed / 22 skipped / 0 failed**, skip count unchanged -- `pnpm check:reachability` **exit 0, 471 releases, 60 distinct plunge strengths, 70 distinct flip ticks, 0 mismatches, 76.6 s** -- `pnpm check:ad7` **exit 1** and `pnpm check:corridor` **exit 1**, both unchanged by design -- `pnpm lint:boundaries` / `check:headers` / `check:attributions` exit 0 -- `git diff --stat` over `assets/src/dragonwar.blend`, `public/assets/dragonwar.glb`, `public/assets/dragonwar.collision.json` and `test/replays/` **empty**, collision-document SHA-256 still `4955324fce88eab4b25f2930b3cf801f88c66c487f4d3b4e1756d3e2c603ecc1` -- all four touched source files contain zero non-ASCII bytes (Rule 14).
...
**QA pass (2026-09-03) -- `bmad-qa-generate-e2e-tests`.** This story's deliverable is itself test infrastructure, so the QA contribution is narrow by design: not more shot cases, but direct tests of the harness's own primitives (`test/util/plan-geometry.ts`, `test/util/reachability.ts`, `test/util/shot-cases.ts`), independent of the 39-case manifest -- none of the story's own 88 in-suite tests exercise these primitives against anything other than the real, already-working geometry, so a bug in the distance math or in either "fails loudly on a bad reference" contract could rot the whole harness's guarantee without a single existing test noticing (precedent: `test/geometry.test.ts`'s own DW-61 header, "the FIRST direct test of the function itself" for a sibling primitive).

New test file: **`test/reachability-harness-integrity.test.ts`** (QA) -- 21 tests, self-time ~2.1 s. Covers:
- `pointToSegmentDistanceMm()` with synthetic geometry (a point on-segment, perpendicular-off, beyond-an-endpoint -- the clamp, not the infinite line -- and a degenerate zero-length segment), and `pointInPolygon()`/`distanceToPolygonMm()` with a simple square -- the primitive every reachability measurement in this story (the in-suite gate, the dense sweep, and the `DW-130` proximity record) reduces to, never directly tested before.
- `witnessPath()`/`closestApproachMm()` fail loudly (throw, naming the id) on an unknown witness id, rather than silently resolving to a distance -- the property standing between a typo'd witness reference and a silently wrong verdict, never exercised because every real manifest entry already names a real witness.
- `closestApproachOverAll()` genuinely discriminates: a point far outside the playfield is reported farther than `REACHABILITY_TOLERANCE_MM`, and (`it.each` over all 10 `witnessIds()`) a point taken from each witness's own last swept segment is found within a hair of 0 mm -- confirming the search actually considers every declared witness, not just the first one in the table.
- `shotCase()` genuinely refuses an undeclared id (throws, naming it) -- the dynamic half of the manifest gate `driveCase()` depends on; the existing suite only ever exercises the source-scan (static) half and the positive path.

**Mutations demonstrated (QA, Rule 19 -- each applied to a `test/util/*.ts` file, red observed with the required content, reverted, tree confirmed byte-identical via `git status --short` / `git diff --stat`):**
- `test/util/plan-geometry.ts`: commented out `pointToSegmentDistanceMm()`'s `t = Math.max(0, Math.min(1, t))` clamp -> two tests went red -- the "beyond one endpoint" test (`expected +0 to be close to 5`) and, as a side effect, this file's own "far outside the playfield" sanity test (the unclamped line distance corrupted an unrelated collinear measurement) -- both reverted, tree confirmed unchanged.
- `test/util/reachability.ts`: `witnessPath()`'s unknown-id branch changed from `throw` to returning an empty `{segments: [], closedSwitches: new Set(), pathLengthMm: 0}` -> both the `witnessPath()`-throws and the `closestApproachMm()`-propagates tests went red (`expected [Function] to throw an error`). Reverted.
- `test/util/shot-cases.ts`: `shotCase()`'s unknown-id branch changed from `throw` to falling back to `SHOT_CASES[0]` -> the "refuses an undeclared id" test went red the same way. Reverted.
- `test/util/reachability.ts`: `closestApproachOverAll()`'s search restricted to `WITNESSES.slice(0, 1)` (only the first witness) -> **first attempt, sampling each witness's own FIRST swept segment, stayed green -- a genuine gap in the test, not a clean pass.** All 10 witnesses share an identical serve/settle prefix (the served ball resting on the plunger tip before any plunge hold begins), so a first-segment sample is nearly the same point for every witness and could not discriminate a broken search from a working one. Fixed by resampling from each witness's own LAST swept segment instead (its point of maximum divergence); re-run of the same mutation then correctly turned 9 of 10 cases red (`expected <large number> to be close to +0`) -- only `plunge-full` (witness index 0 itself) stayed green, exactly as expected. Reverted; the corrected last-segment version is what ships.
- `test/util/shot-cases.ts`: `ramp-return-geometry`'s recorded `closestApproachMm` (an already-`unreachable`, `DW-137` case) changed from `44.979` to `200`, staying well outside `REACHABILITY_TOLERANCE_MM` so the case's *first* per-case assertion (still-unreachable) stayed green -- isolates whether the *second* assertion (recorded-vs-live agreement band) is load-bearing or decorative, a question the developer's own recorded AC 2 mutations did not isolate (theirs flipped reachability kind, not just the recorded number on an already-unreachable case). Result: went red exactly as expected, naming the case, the stale recorded value and the live measurement (`disagrees ... by more than the 0.5 mm agreement band`) -- confirms `closestApproachMm` on an `unreachable` entry is genuinely load-bearing, not decorative. Reverted.

`mutations_demonstrated=5` (QA pass; distinct from the implementer's own `## Spec Change Log` mutations, one per AC, recorded separately above).

**Re-verified after the QA pass:** `pnpm typecheck` clean (all three projects); `pnpm check:headers` / `pnpm check:attributions` OK (new file staged so `git ls-files` sees it, then unstaged again -- QA leaves all changes uncommitted); `pnpm check:ad7` / `pnpm check:corridor` still exit 1, unchanged; `npx vitest run test/shot-routing.test.ts test/shot-reachability.test.ts test/reachability-harness-integrity.test.ts` -- **147 passed** (38 + 88 + 21); `npx vitest run` (full suite, no `BLENDER`) -- **90 files / 1308 passed / 22 skipped** (baseline 89 files / 1287 passed / 22 skipped, delta exactly this QA file's own 21 tests, skip count unchanged); `git diff --stat` over the geometry/golden paths -- empty throughout.

**Judged against the QA brief's five specific questions:** (1) an `unreachable` declaration that a witness actually reaches -- already gated by the per-case test's first assertion (`best.closestApproachMm > REACHABILITY_TOLERANCE_MM`), matching the developer's own recorded inverse AC 2 mutation; not re-demonstrated here (already evidenced). (2) `closestApproachMm` load-bearing vs decorative -- **independently confirmed load-bearing** by the fifth mutation above, a gap the developer's own mutation set had not isolated. (3) a witness id referencing a nonexistent or no-longer-close witness -- covered by three new tests plus two mutations (the unknown-id throw, and the "every witness genuinely considered" `it.each`, whose first draft was itself caught non-discriminating and fixed before shipping). (4) `driveCase()`/`shotCase()` refusing an undeclared coordinate -- confirmed both statically (existing source-scan tests, unchanged) and now dynamically (new `shotCase()` throw test + mutation). (5) `pnpm check:reachability`'s green being meaningful -- verified by code inspection only (the anti-vacuity floor at `MIN_RELEASES_EVALUATED = 300`, checked after a real sweep run, single `it()` block so a floor failure fails the whole file): not independently re-run, since the developer's own AC 6 mutation already demonstrated it red-then-reverted, an in-suite wrapper for this specific harness is a design decision this story explicitly rejected (Design Notes, "Alternatives considered and rejected"), and a fresh run costs ~75 s against a real, unmutated 471-release sweep for no new evidence.
