...
- `package.json`'s fourteen scripts, pnpm 11.24.0 / Vite 8.2.2 / Vitest 4.1.11 / TypeScript 7.0.2 / Node >=24 as the spine fixed them: `dev`, `typecheck` (`tsc --noEmit` over the three tsconfig projects), `test` (`vitest run`), `build`, `preview`, `check:dist`, `check:size`, `lint:boundaries`, `check:headers`, `check:attributions`, `export:assets`, `check:ad7`, `check:corridor`, `check:reachability`. `check:ad7` (DW-70, Story 1.8) is the **only** intended-red check: it is a live, documented defect, wired outside `pnpm test` and CI's fixed script list, with an in-suite wrapper test (`test/ad7-device-slots.test.ts`) that asserts the failure's own content rather than merely its exit code. `check:corridor` (DW-137) **was** the second one, shipped red by Story 2.1c; **Story 2.1f re-solved the bottom-right corridor and it is now intended green** — its in-suite wrapper (`test/dw137-corridor-gate.test.ts`) was removed in the same change, because an intended-red wrapper around a green check asserts nothing. `check:corridor` is now an opt-in green check like `check:reachability`: run it whenever the committed geometry moves. `check:reachability` (Story 2.1e) is different in kind, not degree: it is opt-in and **intended green** — a real check (the dense out-of-process sweep that proves each shot-routing release point reachable, or genuinely not) kept out of `pnpm test` for cost (644 release trajectories, ~105-108 s measured -- 472 until Story 2.1f added the `side` axis for the right-bat origin, `plunge-then-bat-r-3890`/`-3899`/`-3906`, plus `plunge-then-bat-l-3969`, taking the `WITNESSES` corpus to 15 and the sweep to 2 bat sides / 644 recipes), not because it documents a live defect. **Run it whenever the committed geometry moves** (any `pnpm export:assets`, any edit to `tools/make-placeholder-blend.py`), whenever a case is added to or moved in `test/util/shot-cases.ts`, and whenever a story changes an `unreachable` verdict — it is in neither CI nor `pnpm test`, and it is not typechecked (`tsconfig.node.json` excludes `test/fixtures/**`), so nothing else will notice it rotting.
- The Blender-gated tests and `export:assets` need Blender on `PATH` or, more reliably, the `BLENDER` environment variable set to the executable's full path — `resolveBlender()` (`tools/blender.mjs`) checks `BLENDER` first and uses it verbatim. Set it persistently for your user rather than per shell, so every agent process inherits it; without it each export-touching stage re-runs discovery, and a portable install lands outside every conventional location the resolver probes (DW-132). **Never commit the path** — a host path in a public repository is DW-131, and a hardcoded one is the DW-46 defect. Blender-gated tests skip cleanly when it is unset (CI included), so an unset `BLENDER` shows up as skips, not failures.
